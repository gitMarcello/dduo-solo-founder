...
also needs Docker Desktop. The host supplies an OpenAI embeddings key and
official Codex subscription login for consolidation, even when chatting with
Claude. The installer prepares `uv`, not Docker or client CLIs.
...
One project-owned Codex executor uses `gpt-5.6-terra` with medium reasoning.
Codex and Claude turns share the queue while retaining their source client.
...
consolidation. There is no generative API fallback or automatic subscription
switch. Rate limits retry; authentication failures need login. See
