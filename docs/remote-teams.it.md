...
Il consolidamento remoto usa il bridge server e il login Codex file-backed
privato del progetto, modello `gpt-5.6-terra` e reasoning `medium`. I
collaboratori non ricevono credenziali VPS o provider. Il processo è effimero,
senza strumenti shell/exec, ricerca web o checkout: il testo non fidato della
memoria non può diventare un agente che legge credenziali locali. Se il login
scade o viene revocato, il gestore autentica di nuovo il Codex home del progetto.
...
`login-codex --device-auth` ufficiale, senza inventare sessioni o dichiarare
