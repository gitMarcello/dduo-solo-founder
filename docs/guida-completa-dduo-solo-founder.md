...
e un solo login in abbonamento per consolidare: Codex oppure Claude. La prima
chat supportata del progetto stabilisce la preferenza; l'host la verifica prima
di ogni passaggio e può usare l'altro abbonamento già verificato sull'host solo
quando quello preferito non è disponibile. L'installer prepara `uv`, non Docker
o le CLI dei client.
