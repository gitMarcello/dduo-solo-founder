...
flowchart LR
  ...
  B --> A["Host agent: Setup and automatic sleep"]
  B --> P["Project API"]
  P --> DB[("PostgreSQL")]
...
sign-in requests, project-scoped OpenAI and file-backed Codex or Claude
credentials, and resuming auth-paused jobs. Docker receives an authenticated
host-agent token, never a provider subscription credential. The first supported
session sets a project preference; immediately before each sleep pass the bridge
uses that verified host subscription, falling back to the other verified host
subscription only when the preferred login or executable is unavailable. It
does not switch subscriptions for a rate limit, timeout or invalid model output.
Codex runs pin `gpt-5.6-terra` with medium reasoning. Both runners have no
shell/exec tools, web search or project checkout, so untrusted memory text
cannot turn consolidation into a credential-reading agent.
...
Pending jobs from either client converge in one durable queue. The first
supported project chat establishes a Codex or Claude preference; the host keeps
`source_client` as provenance and records the executor that actually ran every
attempt. Before a pass it may use the other already-verified host subscription
only when the preferred login or executable is unavailable, never to bypass a
quota, timeout or invalid output. A paused job never blocks interactive work.
