...
flowchart LR
  ...
  B --> A["Agente host: Setup e sleep automatico"]
...
L'agente gestisce solo funzioni dell'host: chiamate seriali alle CLI in
abbonamento, Setup, accessi nativi, chiavi OpenAI e credenziali Codex o Claude
su file isolate per progetto, ripresa dei job sospesi per autenticazione. Docker
riceve il token dell'agente host, mai la credenziale dell'abbonamento. La prima
sessione supportata imposta una preferenza Codex o Claude; prima di ogni sonno
il bridge usa quell'abbonamento verificato sull'host e ricorre all'altro già
verificato solo se login o eseguibile preferito non sono disponibili. Non cambia
abbonamento per aggirare limiti, timeout o output non valido.

Le esecuzioni Codex usano `gpt-5.6-terra` con ragionamento medio. Entrambi i
runner sono effimeri, senza shell/exec, ricerca web o checkout: il modello
riceve input limitato e schema strutturato, senza strumenti per leggere le
credenziali dell'host.
...
all'host del progetto: i job di entrambi convergono in una sola coda, con una
preferenza Codex o Claude stabilita dalla prima chat supportata e
`source_client` conservato come provenienza. Ogni job registra anche l'esecutore
effettivo. La coda durevole e il confine di autenticazione sono unici. Un job
sospeso non blocca il lavoro interattivo.
