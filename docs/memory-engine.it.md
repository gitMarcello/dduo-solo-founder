...
L'esecutore attuale è Codex sull'host, con `gpt-5.6-terra`, ragionamento medio e
un `CODEX_HOME` privato di progetto con credenziali su file. I job nuovi e quelli
già in coda vengono normalizzati su questo esecutore. `source_client` conserva
Codex o Claude come provenienza, senza scegliere un altro provider o dividere
la coda. La memoria risultante appartiene al progetto ed è condivisa subito
tra i due client. Non esistono fallback generativi automatici su API a pagamento
o memorie temporanee per il passaggio di consegne.
...
Testo dell'assistente, artefatti, citazioni e codice non decidono la lingua.

Il consolidatore restituisce `it` o `en` come valore strutturato e il server
valida il testo proposto prima della scrittura. Chiavi stabili e ID sorgente
