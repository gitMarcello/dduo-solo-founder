...
path, not something a manifest alone proves. Do not call an IDE surface
supported until it has passed the short real-chat procedure below on that
extension version and operating system. This deliberately excludes WSL, Remote
SSH and Dev Containers; a normal checkout bound to a VPS is included.

| Surface | macOS / Windows | What proves it |
| --- | --- | --- |
| Codex CLI | supported | native readiness and a saved turn |
| Claude Code CLI | supported | native readiness and a saved turn |
| Codex VS Code | acceptance required | graphical chat emits context and saves two turns |
| Claude Code VS Code | acceptance required | graphical chat emits context and saves two turns |

**Codex IDE limitation (checked 8 September 2026):** the [official plugin
documentation](https://learn.chatgpt.com/docs/plugins) still states that the IDE
extension does not support plugins. A successful installer or an embedded
runtime reporting approved hooks does not override that limitation. Do not
distribute this Beta as VS Code-certified without the graphical acceptance
evidence for the installed version. No separate VS Code MCP configuration or
hidden lifecycle fallback is provided.

## Verify an official VS Code integration

Use an empty, artificial test repository and a dedicated dDuo binding. Never
use a real project or memory for this check. Record VS Code, extension and
client versions; do not record account data, project paths or chat text.

Install exactly one selected adapter with its real client configuration:

```bash
node bin/install.mjs --only codex --surface vscode --project-root <TEST-REPOSITORY> --yes
# Or: --only claude --surface vscode
```

`--surface vscode` changes instructions and diagnostics only: it is not proof
that the IDE invoked the hooks. Reload the VS Code window, complete any native
approval offered by that client, and open a **new graphical conversation**.
Use a unique artificial phrase stored only in dDuo test memory, then ask a
question which can be answered only if that phrase was injected. Complete two
turns and verify that each is saved once, alongside SessionStart,
UserPromptSubmit and Stop evidence for the same native session. Also perform
one MCP operation and a consolidation on the memory host. The full capture
template and pass/fail rules are in [VS Code lifecycle validation](vscode-lifecycle-validation.md).
...
access, Git, Node.js 18+ and Codex CLI 0.150.0+ or Claude Code. The dDuo
installer installs `uv`; it does not install Docker, Node.js, Git or either
sleep client.
...
Continue through private input and one official subscription login:

```bash
dduo-solo-founder configure-openai --project-root .
dduo-solo-founder login-codex --device-auth --project-root .
# Or, when this project will sleep through Claude:
# dduo-solo-founder login-claude --project-root .
dduo-solo-founder init --headless --yes --project-root .
```

The first command prompts without echo for the embeddings key and stores it
outside Git. Choose one of the next two commands: Codex logs in to the
project's private file-backed Codex home; Claude logs in to its private Claude
configuration directory. The manager completes the official authorization
shown by that process. The first supported project chat records its preferred
sleep provider; each pass checks the verified server login and may use the
other already-verified server subscription only if the preferred provider is
unavailable before model output.
...
and supported sleep-login checks, installs and verifies the persistent user agent, secures
...
For a CLI, start a new session. For VS Code, reload the window and open a new
graphical chat; a Codex Desktop installation keeps its separate desktop
instructions. Verify any actual native consent still required. Create collaborator invitations from the
