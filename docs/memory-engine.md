...
The current server-owned executor is Codex, pinned to `gpt-5.6-terra` with
medium reasoning and a private, project-scoped file-backed `CODEX_HOME`. New
and previously queued jobs are normalized to that executor. `source_client`
retains whether the source session was Codex or Claude for provenance, but it
does not select another sleep provider or split the backlog. The resulting
memory belongs to the project and is immediately shared across both clients.
There is no paid generative API fallback or temporary handoff memory.
...
Assistant prose, artifacts, quoted text and code do not decide the language.

The consolidator must return the structured value `it` or `en`, and the server
validates the proposed memory text against the selected language before writing
it. Stable node keys and source IDs remain unchanged, so language does not split
one fact into parallel memories. The policy applies only to newly consolidated
