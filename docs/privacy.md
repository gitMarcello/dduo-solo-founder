...
the host's preferred Codex or Claude subscription executor, in a private
project configuration directory. Source-client identity is provenance; the
host may use the other already-verified subscription only when the preferred
login or executable is unavailable before model output.
...
`configure-openai` input and official `login-codex --device-auth` or
`login-claude`, keeping
