...
Setup usa, per esempio, `./install.sh --only codex --no-setup`. Queste opzioni
dell'installer non inizializzano la memoria del progetto.
...
`configure-openai` nasconde la chiave digitata; il login Codex usa l'archivio
privato delle credenziali in abbonamento del progetto. Installa prima sul
server una CLI Codex compatibile, anche se la workstation usa Claude. Non
...
Nell'app desktop apri **Settings > Hooks**, seleziona **dDuo Solo Founder**,
poi **Review** e **Trust all**. `/hooks` è un comando della CLI Codex, non un
comando chat. Dopo installazione o aggiornamento, chiudi completamente e
riapri Codex prima di iniziare una nuova chat: la sola chat nuova non ricarica
il pacchetto.

Se Setup segnala un ciclo di vita incompleto anziché chiedere fiducia,
aggiorna o reinstalla dDuo, riavvia Codex e riapri Setup. Non continuare a
...
riavvia completamente Codex e riapri Setup. Non modificare gli hook installati
...
da Setup, ripeti il controllo, riavvia completamente Codex e apri una nuova
chat. Non modificare il manifesto installato. Claude non usa questo campo e
...
La prima configurazione da Claude può richiedere un login in abbonamento Codex
sull'host della memoria: Codex consolida il materiale di entrambi i client e
`source_client` è solo provenienza. Una richiesta di usare Claude come altro
esecutore di consolidazione o una seconda coda provider non è prevista. Dopo
l'aggiornamento apri una nuova chat; se il problema resta, usa la diagnosi
indicata in fondo. Non cancellare lavori pendenti né cambiarne a mano il provider.
...
esegui esplicitamente l'installer e completa Setup. Se hai aggiornato Codex,
riavvialo completamente, poi apri una nuova chat/sessione nello stesso progetto.
