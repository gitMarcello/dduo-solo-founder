...
def fixture_client(client: str, args: list[str]) -> None:
    if client == "claude" and args[:1] == ["plugin"]:
        assert Path(os.environ["CLAUDE_CONFIG_DIR"]) == Path.home() / ".claude"
    ...
    with tempfile.TemporaryDirectory(prefix="dduo-native-smoke-") as temporary:
        ...
        if os.name == "nt":
            ...
            for directory in (codex, native_bin, claude):
                directory.mkdir(parents=True)
            (codex / "package.json").write_text(json.dumps({
                "name": "@openai/codex", "bin": {"codex": "cli.cjs"},
            }), encoding="utf-8")
            ...
            (codex / "cli.cjs").write_text(
                "const {spawnSync}=require('node:child_process');"
                f"const r=spawnSync({json.dumps(str(native_bin / 'codex.exe'))},"
                "process.argv.slice(2),{stdio:'inherit'});process.exit(r.status??1);",
                encoding="utf-8",
            )
        ...
        if not simulate_runtime:
            ...
            run([str(native_bin / f"python{extension}"), "-c", (
                ...
                "codex_status=subscription_auth_status('codex',installation=CliRunner._installation('codex')); "
                "assert codex_status.ready, codex_status.as_dict(); "
                "claude_status=subscription_auth_status('claude',installation=CliRunner._installation('claude')); "
                "assert claude_status.ready, claude_status.as_dict(); "
