...
def _validate_generated_language(value: str, expected: MemoryLanguage, *, field: str) -> None:
    ...
    if detected is not None and detected != expected:
        raise SleepProviderError(
            ...
            f"The memory model returned {field} in the wrong language.",
        )
...
def sleep_executor_provider() -> str:
    """Return the server-owned executor used for every project's sleep work."""
    provider = os.getenv("DDUO_SLEEP_EXECUTOR_PROVIDER", "codex").strip().lower()
    ...
    db.add(
        Activity(
            ...
            detail={
                ...
                "source_client": session.client,
                "executor_provider": job.provider,
            },
            actor="system",
            actor_member_id=session.member_id,
        )
...
async def resume_project_sleep(
    ...
    *,
    ...
    """Resume due transient work, plus an authenticated provider explicitly confirmed in Setup."""
    executor = sleep_executor_provider()
    active_jobs = list(
        (
            await db.scalars(
                select(SleepJob)
    ...
    if provider:
        waiting_statement = waiting_statement.where(SleepJob.provider == provider)
    waiting = list(
    ...
    recoverable = [
        ...
        or (
            ...
            and (provider is None or job.provider == provider)
        )
    ]
...
async def process_one_sleep_job(
    ...
    job.provider = sleep_executor_provider()
    job_id = job.id
    ...
    try:
        ...
        for topic in consolidation.topics:
            ...
            if topic.language != topic_language:
                raise SleepProviderError(
                    "invalid_model_output",
                    "The memory model returned a topic language that conflicts with its sources.",
                )
            _validate_generated_language(topic.label, topic_language, field="a topic label")
            _validate_generated_language(topic.query_text, topic_language, field="a topic query")
            actions = []
            for action in topic.memory_actions:
                ...
                action_language = _resolved_action_language(
                    language_hint, sanitized_action, related_by_id
                )
                if action_language != topic_language:
                    raise SleepProviderError(
                        "invalid_model_output",
                        "The memory model mixed incompatible languages in one topic.",
                    )
        ...
        totals = {"topics": 0, "created": 0, "replaced": 0, "status_updated": 0, "skipped": 0}
        for item in planned:
            serialized_actions = []
