...
def portable_file_lock(registry_path: Path, timeout: float = 5) -> Iterator[None]:
    ...
    while descriptor is None:
        ...
        try:
            ...
            # Windows permits byte-range locking beyond EOF. Acquire before
            # any write/truncate: another opener can already own byte zero
            # even when our earlier stat observed an empty file.
