...
def external_session_id(payload: dict) -> str | None:
    """Return a client-provided session identity, never a shared fallback."""
...
def state_path(
    ...
    seed = f"{binding_id}\0{client}\0{external_id}" if binding_id else external_id
    digest = hashlib.sha256(seed.encode("utf-8")).hexdigest()[:20]
    suffix = f"-{binding_id[:12]}" if binding_id else ""
    return HOOK_STATE_DIR / f"{project_id}-{client}{suffix}-{safe}-{digest}.json"
...
def load_bound_state(
    ...
    """Load only state belonging to this exact project/client/binding/session.

    Old state files without a binding are intentionally not upgraded in place:
    after a remote move or restore they cannot prove ownership of the current
    memory authority.  They remain intact for manual recovery rather than
    risking a cross-project replay.
    """
    path = state_path(project_id, client, external_id, binding_id)
    with portable_file_lock(path, timeout=1):
        # Existence, not truthiness, makes migration one-shot. An empty or
        # damaged new ledger must never resurrect an already delivered queue.
        if path.exists():
            return path, read_state(path)
        legacy = state_path(project_id, client, external_id)
        if not legacy.exists():
            return path, {}
        with portable_file_lock(legacy, timeout=1):
            legacy_state = read_state(legacy)
            if (
                legacy_state.get("project_id") == project_id
                and legacy_state.get("binding_id") == binding_id
                and legacy_state.get("client", client) == client
                and legacy_state.get("session_external_id") == external_id
            ):
                # The exact legacy filename plus all three stored identities
                # permits inferring a family omitted by older releases.
                legacy_state["client"] = client
                # Older local captures omitted per-item binding metadata. Only
                # this fully qualified migration may inherit the ledger's
                # authority. A foreign explicit binding is never rewritten.
                for key in ("pending_commits", "spooled_turns"):
                    items = legacy_state.get(key)
                    for item in items if isinstance(items, list) else []:
                        if isinstance(item, dict) and item.get("binding_id") is None:
                            item["binding_id"] = binding_id
                active = legacy_state.get("spooled_active")
                if isinstance(active, dict) and active.get("binding_id") is None:
                    active["binding_id"] = binding_id
                _invalidate_delivery_baseline(legacy_state)
                legacy_state.pop("delivery_baseline", None)
                legacy_state.pop("lifecycle_evidence", None)
                # Materialize the entire import before an optimistic merge
                # can mistake unchanged legacy fields for persisted fields.
                write_state(path, legacy_state)
                return path, legacy_state
        print("dDuo: unqualified legacy hook state preserved for manual recovery.", file=sys.stderr)
        return path, {}
...
_STATE_LIST_IDENTITIES = {
    ...
    # A retry acknowledgement must remove the exact snapshot it sent, never a
    # newer revision staged by a concurrent hook for the same server turn.
    "pending_commits": lambda item: str(
        item.get("revision") or f"legacy:{item.get('turn_id') or ''}"
    ),
...
def merge_state_update(
    path: Path, before: dict, after: dict, *, lock_timeout: float = 10
) -> dict:
    ...
    """
    with portable_file_lock(path, timeout=lock_timeout):
            ...
            if (
                ...
                merged = [
                    ...
                    if not (
                        ...
                        and item == old_by_id[identify(item)]
                    )
                ]
                positions = {
                    identify(item): index
                ...
                for identity, item in new_by_id.items():
                    ...
                    if identity in positions:
                        ...
                        if existing == old_by_id.get(identity):
                            merged[positions[identity]] = item
                        elif key in {"pending_commits", "spooled_turns"}:
                            merged[positions[identity]], conflict = _merge_completion_snapshot(
                                existing, item, nested=key == "pending_commits"
                            )
                            if conflict:
                                _record_stop_conflict(current)
                    else:
    ...
    *, deadline: float | None = None,
    ...
    try:
        ...
        response = _request(
            ...
            json={"items": items},
            timeout=_recovery_timeout(deadline) if deadline is not None else 3,
        )
...
def drain_pending_codex_usage(
    state: dict, *, project_id: str, deadline: float | None = None,
    ...
    for index, source in enumerate(pending):
        if (
            (max_sources is not None and index >= max_sources)
...
def _commit_payload_has_response(payload: object) -> bool:
    return isinstance(payload, dict) and isinstance(payload.get("assistant_response"), str)


def _record_stop_conflict(
    state: dict, *, external_turn_id: str | None = None, turn_id: str | None = None,
) -> None:
    evidence = state.setdefault("lifecycle_evidence", {"version": 1})
    if isinstance(evidence, dict):
        stop_evidence = evidence.setdefault("stop", {})
        if isinstance(stop_evidence, dict):
            stop_evidence["conflict"] = True
            if external_turn_id is not None:
                stop_evidence["external_turn_id"] = external_turn_id
            if turn_id is not None:
                stop_evidence["turn_id"] = turn_id
    print("dDuo: conflicting Stop answers received; the first captured answer was preserved.", file=sys.stderr)


def _merge_completion_snapshot(left: dict, right: dict, *, nested: bool) -> tuple[dict, bool]:
    """Preserve the first valid answer, enriching only its captured metadata.

    Observation additions are not conflicting assistant answers. Every changed
    pending snapshot gets a new revision so an earlier in-flight ACK cannot
    remove the additional evidence. Privacy can become stricter, never looser.
    """
    merged = deepcopy(left)
    old_body = left.get("payload") if nested else left
    new_body = right.get("payload") if nested else right
    old_body = old_body if isinstance(old_body, dict) else {}
    new_body = new_body if isinstance(new_body, dict) else {}
    body = deepcopy(old_body)
    conflict = bool(
        _commit_payload_has_response(old_body)
        and _commit_payload_has_response(new_body)
        and old_body["assistant_response"] != new_body["assistant_response"]
    )
    if not _commit_payload_has_response(old_body) and _commit_payload_has_response(new_body):
        body["assistant_response"] = new_body["assistant_response"]
    observations = []
    for source in (old_body, new_body):
        values = source.get("context_observations")
        for observation in values if isinstance(values, list) else []:
            if isinstance(observation, dict) and observation not in observations:
    ...
    if nested:
        ...
        merged.update(body)
    for key in ("external_turn_id", "session_external_id", "binding_id"):
        if key not in merged and key in right:
            merged[key] = right[key]
    if "off_record" in left or "off_record" in right:
        merged["off_record"] = bool(left.get("off_record")) or bool(right.get("off_record"))
    if nested and not _same_pending_commit(merged, left):
        merged["revision"] = str(uuid.uuid4())
    return merged, conflict


def _same_pending_commit(left: dict, right: dict) -> bool:
    """Compare the delivery semantics of two snapshots.

    ``revision`` and ``queued_at`` identify one local delivery attempt, not
    the captured completion.  A duplicate Stop event therefore remains
    idempotent even though it necessarily receives a new local timestamp.
    """
    ignored = {"revision", "queued_at"}
    return (
...
def queue_pending_commit(
    ...
    *, external_turn_id: str | None = None, off_record: bool | None = None,
) -> dict:
    """Stage a completion before delivery and return its immutable snapshot.

    The first valid response wins when duplicate Stop events disagree.  This
    avoids silently overwriting captured project content while still allowing
    an identical retry to remain idempotent.
    """
    candidate = {
        "turn_id": turn_id,
    ...
    if external_turn_id is not None:
        candidate["external_turn_id"] = external_turn_id
    if off_record is not None:
        candidate["off_record"] = off_record

    pending = _pending_commits(state)
    for index, item in enumerate(pending):
        if item.get("turn_id") != turn_id or item.get("binding_id") != candidate.get("binding_id"):
            continue
        merged, conflict = _merge_completion_snapshot(item, candidate, nested=True)
        if conflict:
            _record_stop_conflict(state, external_turn_id=external_turn_id, turn_id=turn_id)
        pending[index] = merged
        state["pending_commits"] = pending
        return merged
    state["pending_commits"] = [*pending, candidate]
    return candidate


def _stop_timeout() -> httpx.Timeout:
    """Bound one delivery without consuming the native Stop hook budget."""
    return httpx.Timeout(connect=1.0, write=1.0, read=3.0, pool=0.5)


def _committed_stop_response(response: object) -> bool:
    """A successful HTTP status is insufficient to prove a stored turn."""
    try:
        response.raise_for_status()  # type: ignore[attr-defined]
        body = response.json()  # type: ignore[attr-defined]
    except Exception:
        return False
    return isinstance(body, dict) and body.get("committed") is True


def _remove_pending_commit_if_same(state: dict, item: dict) -> bool:
    """Delete only the revision acknowledged by the server."""
    remaining = [candidate for candidate in _pending_commits(state) if candidate != item]
    if len(remaining) == len(_pending_commits(state)):
        return False
    if remaining:
...
def _ack_pending_commit(path: Path, item: dict) -> bool:
    """Check and remove an acknowledged snapshot in one critical section."""
    with portable_file_lock(path, timeout=1):
        current = read_state(path)
        if not _remove_pending_commit_if_same(current, item):
            return False
        evidence = current.get("lifecycle_evidence")
        stop_evidence = evidence.get("stop") if isinstance(evidence, dict) else None
        if isinstance(stop_evidence, dict) and stop_evidence.get("turn_id") == item.get("turn_id"):
            stop_evidence["server_committed_at"] = datetime.now().astimezone().isoformat()
        write_state(path, current)
        return True


def _recovery_timeout(deadline: float | None) -> httpx.Timeout:
    if deadline is None:
        return _stop_timeout()
    remaining = deadline - time.monotonic()
    if remaining <= 0:
        raise TimeoutError("lifecycle recovery budget exhausted")
    # A small bounded maintenance window, independent of retrieval's timeout.
    return httpx.Timeout(min(remaining / 4, 0.5))
    ...
    *,
    ...
    for item in _pending_commits(state):
        ...
        if binding_id and item_binding not in (
            ...
            continue
        if (
            not _commit_payload_has_response(item.get("payload"))
            or (max_items is not None and attempted >= max_items)
            or (deadline is not None and time.monotonic() >= deadline)
        ):
            remaining.append(item)
            continue
        try:
            ...
            response = _request(
                ...
                timeout=_recovery_timeout(deadline),
            )
            if not _committed_stop_response(response):
                raise RuntimeError("stop-check did not confirm persistence")
            persisted += 1
            if path is not None:
                _ack_pending_commit(path, item)
        except Exception:
            remaining.append(item)
    if remaining:
        state["pending_commits"] = remaining
...
def flush_spooled_turns(
    ...
    *,
    ...
    for path in HOOK_STATE_DIR.glob(f"{project['id']}-{client}-*.json"):
        ...
        if binding_id and state.get("binding_id") not in (
            {binding_id, None} if allow_legacy else {binding_id}
        ):
            continue
        if not _spooled_turns(state):
            continue
        ...
        for item in _spooled_turns(state):
            ...
            if binding_id and item_binding not in (
                ...
                continue
            if (
                not _commit_payload_has_response(item)
                or (max_items is not None and attempted >= max_items)
                or (deadline is not None and time.monotonic() >= deadline)
            ):
                remaining.append(item)
            ...
            try:
                attempted += 1
                session_options = {"timeout": _recovery_timeout(deadline)} if deadline is not None else {}
                session = _session(
                    transport, project, client, str(item["session_external_id"]), **session_options
                )
                turn = None
                for fragment in _spooled_prompt_fragments(item):
                    turn = _request(
                        ...
                        timeout=_recovery_timeout(deadline) if deadline is not None else 30,
                    )
                ...
                if not _committed_stop_response(response):
                    raise RuntimeError("stop-check did not confirm replay persistence")
                persisted += 1
                evidence = state.get("lifecycle_evidence")
                stop_evidence = evidence.get("stop") if isinstance(evidence, dict) else None
                if isinstance(stop_evidence, dict) and stop_evidence.get("external_turn_id") == item.get("external_turn_id"):
                    stop_evidence["turn_id"] = str(turn.json()["turn"]["id"])
                    stop_evidence["server_committed_at"] = datetime.now().astimezone().isoformat()
            except Exception:
                remaining.append(item)
        if remaining:
        ...
        merge_state_update(path, before, state, lock_timeout=1)
    return persisted


def _recover_after_context(
    project: dict, transport: str | ProjectHttpClient, client: str,
    binding_id: str, path: Path,
) -> None:
    """Bound maintenance after context emission; never spend retrieval's budget.

    At most one pending completion, one offline turn and one usage source are
    attempted per event, sharing one two-second maintenance window. Interrupted
    attempts retain their immutable outboxes for the next event.
    """
    ...
    try:
        ...
        drain_pending_codex_usage(
            state, project_id=str(project["id"]), deadline=deadline, max_sources=1
        )
        merge_state_update(path, before, state, lock_timeout=1)
        _recover_one_pending_completion(project, transport, client, binding_id, path, deadline)
        flush_spooled_turns(
            project, transport, client, binding_id, allow_legacy=False,
            deadline=deadline, max_items=1,
        )
        if time.monotonic() < deadline:
            flush_client_telemetry(transport, str(project["id"]), deadline=deadline)
    except Exception:
        # Context is already delivered. Maintenance never emits a second hook
        # envelope or claims queued content was committed.
        pass


def _recover_one_pending_completion(
    project: dict, transport: str | ProjectHttpClient, client: str,
    binding_id: str, current_path: Path, deadline: float,
) -> None:
    """Recover one exact snapshot even when its original chat is no longer open.

    Restored collision sidecars contain durable queues, not live-chat evidence.
    They are eligible for replay under their own lock, never selected as the
    current conversation. Incomplete entries do not block later valid entries.
    """
    seen: set[Path] = set()
    paths = chain((current_path,), HOOK_STATE_DIR.glob(f"{project['id']}-{client}-*.json"))
    for path in paths:
        if time.monotonic() >= deadline:
            return
        if path in seen:
        ...
        seen.add(path)
        state = read_state(path)
        native_session = external_session_id({"session_id": state.get("session_external_id")})
        if (
            state.get("project_id") != str(project["id"])
            or state.get("binding_id") != binding_id
            or not native_session or native_session != state.get("session_external_id")
        ):
            continue
        legacy = state_path(str(project["id"]), client, native_session)
        if path == legacy and state.get("client", client) == client:
            # Safely migrate a closed session too; waiting for its next prompt
            # would strand the final turn that was captured before an upgrade.
            path, state = load_bound_state(str(project["id"]), client, native_session, binding_id)
        expected = state_path(str(project["id"]), client, native_session, binding_id)
        suffix = path.stem.removeprefix(expected.stem + "-restored-")
        sidecar = path.stem.startswith(expected.stem + "-restored-") and (
            len(suffix) == 8 and all(character in "0123456789abcdef" for character in suffix)
        )
        if state.get("client") != client or (path != expected and not sidecar):
            continue
        if not any(
            item.get("binding_id") == binding_id and _commit_payload_has_response(item.get("payload"))
            for item in _pending_commits(state)
        ):
            continue
        flush_pending_commits(
            transport, state, binding_id, allow_legacy=False,
            deadline=deadline, max_items=1, path=path,
        )
        return  # At most one network attempt across every session and sidecar.
        ...
        fragments.append(
            ContextFragment(
                ...
                payload={
                    ...
                    **({"dduo_turn_id": turn_id} if turn_id else {}),
...
def session_start() -> None:
    ...
    if not config_path.exists():
        ...
        emit(
            ...
            "one concise question in the user's language, recommending setup first: Shall I get everything ready "
            ...
            "After approval, call open_setup and follow its result without showing commands, IDs, URLs, or "
            "setup internals. If requires_key_choice is returned, ask whether to use the existing OpenAI key "
            "or enter another; wait for the choice before calling open_setup with the matching argument. "
            "Reuse ready prerequisites; do not claim a page opened unless opened is true. Only if opened, ask "
            "the founder to complete the remaining actions and reply 'fatto', then call check_setup with the "
            "current client. If open_setup or check_setup returns ready, state only "
    ...
    if external is None:
        emit(
            "dDuo cannot identify this chat session, so it will not attach memory or claim this "
            "conversation is recorded. Offer to open a new chat after checking the plugin connection.",
            "SessionStart",
            warn=True,
        )
        return
    try:
        ...
        path, _existing_state = load_bound_state(
            str(project["id"]), client, external, binding.binding_id
        )
        ...
        # Maintenance runs after the briefing; a large offline outbox must not
        # starve the context that this session is waiting for.
        state = read_state(path)
        ...
        if baseline is None:
            ...
            delivery_reason = "state_missing" if resume_requested else session_reason
        ...
        try:
            # ``emit`` returning is the strongest evidence available locally:
            # it proves the hook wrote the native envelope, not that an IDE
            # necessarily accepted it or that the model consumed it.
            evidence = state.setdefault("lifecycle_evidence", {"version": 1})
            if isinstance(evidence, dict):
                ...
                evidence["session_start"] = {
                    ...
                    "context_emitted_at": observed_at,
                }
            ...
            pass
        _recover_after_context(project, transport, client, binding.binding_id, path)
        ...
        try:
            ...
            path, state = load_bound_state(
                str(project["id"]), client, external, fallback_binding.binding_id
            )
            try:
...
def user_prompt_submit() -> None:
    ...
    try:
        ...
        # Persist the current turn cursor without draining unrelated queues.
        merge_state_update(path, state_before, state)
        state = read_state(path)
        state_before = deepcopy(state)
        ...
        state.update(
            {
                ...
                "turn_off_record": session_off_record,
            }
        )
        state.pop("spooled_active", None)
        full_compact = {
        ...
        try:
            # This records only lifecycle metadata. It deliberately does not
            # duplicate the prompt or the emitted context in diagnostics.
            evidence = state.setdefault("lifecycle_evidence", {"version": 1})
            if isinstance(evidence, dict):
                observed_at = datetime.now().astimezone().isoformat()
                evidence["version"] = 1
...
def _stage_stop(
    path: Path, payload: dict, *, project_id: str, client: str,
    external_session: str, binding_id: str,
) -> dict | None:
    """Reread, correlate and durably stage exactly one completion under one lock."""
    with portable_file_lock(path, timeout=1):
        state = read_state(path)
        if state.get("binding_id") not in {None, binding_id}:
            raise RuntimeError("project binding changed while this hook session was open")
        declared = payload.get("turn_id")
        ...
        active_external = state.get("external_turn_id") if turn_id else (
            active.get("external_turn_id") if active else None
        )
        targets_active = bool((turn_id or active) and (
            declared is None or declared == active_external
        ))
        pending = next((
            item for item in _pending_commits(state)
            if declared is not None and item.get("external_turn_id") == declared
            and item.get("binding_id") == binding_id
        ), None)
        spooled = next((
            item for item in _spooled_turns(state)
            if declared is not None and item.get("external_turn_id") == declared
            and item.get("session_external_id") == external_session
            and item.get("binding_id") == binding_id
        ), None)
        # A late event without an exact queued target never borrows B's active
        # internal id, observations, cursor or privacy.
        if not targets_active and pending is None and spooled is None and declared is not None and active_external:
            print("dDuo: unmatched Stop preserved the current turn; capture is unverified.", file=sys.stderr)
            return None
        if not targets_active and pending is None and spooled is None:
            return None

        state.update({
            "project_id": project_id, "client": client, "binding_id": binding_id,
            "session_external_id": external_session,
        })
        response_field = next((
        ...
        internal_turn = str(turn_id) if targets_active and turn_id else (
            str(pending["turn_id"]) if pending else None
        )
        observations = []
        if targets_active:
            values = state.get("pending_context_observations")
            observations = [
                deepcopy(item) for item in values if isinstance(item, dict)
                and item.get("turn_id") in {None, internal_turn}
        ...
        captured = False
        if internal_turn:
            privacy = pending.get("off_record") if pending else state.get(
                "turn_off_record", state.get("session_off_record")
            )
            staged = queue_pending_commit(
                state, internal_turn, body, binding_id,
                external_turn_id=native_turn,
                off_record=privacy if isinstance(privacy, bool) else True,
            )
            captured = True
        elif (targets_active and active) or spooled:
            source = active if targets_active and active else spooled
            item = deepcopy(source)
            item["binding_id"] = binding_id
            # Captured privacy belongs to the turn, not today's session.
            item["off_record"] = item.get("off_record") if isinstance(item.get("off_record"), bool) else True
            item, conflict = _merge_completion_snapshot(item, body, nested=False)
            if conflict:
                _record_stop_conflict(state, external_turn_id=native_turn)
            queued = _spooled_turns(state)
            index = next((
                index for index, entry in enumerate(queued)
                if entry.get("session_external_id") == external_session
                and entry.get("external_turn_id") == native_turn
                and entry.get("binding_id") == binding_id
            ), None)
            if index is None:
                queued.append(item)
            else:
                queued[index], conflict = _merge_completion_snapshot(queued[index], item, nested=False)
                if conflict:
                    _record_stop_conflict(state, external_turn_id=native_turn)
            state["spooled_turns"] = queued
            captured = True
        if captured:
            evidence = state.setdefault("lifecycle_evidence", {"version": 1})
            if isinstance(evidence, dict):
                previous = evidence.get("stop")
                previous = previous if isinstance(previous, dict) else {}
                available = _commit_payload_has_response(staged.get("payload")) if staged else (
                    _commit_payload_has_response(item)
                )
                evidence["version"] = 1
                evidence["stop"] = {
                    "observed_at": datetime.now().astimezone().isoformat(),
                    "external_turn_id": native_turn,
                    **({"turn_id": internal_turn} if internal_turn else {}),
                    "response_available": available,
                    "off_record": (staged if staged else item).get("off_record") is True,
                    **({"queued_at": staged["queued_at"]} if staged and staged.get("queued_at") else {}),
                    **({"conflict": True} if previous.get("conflict") and previous.get("external_turn_id") == native_turn else {}),
                }
            if response_field is None:
                incomplete = state.get("incomplete_stops")
                incomplete = incomplete if isinstance(incomplete, list) else []
                if not any(isinstance(item, dict) and item.get("external_turn_id") == native_turn for item in incomplete):
                    incomplete.append({
                        "external_turn_id": native_turn,
                        **({"turn_id": internal_turn} if internal_turn else {}),
                        "observed_at": datetime.now().astimezone().isoformat(),
                    })
                state["incomplete_stops"] = incomplete[-20:]
        if targets_active:
            source = codex_telemetry_source(payload, dict(state), external_session=external_session)
            if source is not None:
                sources = state.get("pending_codex_usage")
                ...
                state["pending_codex_usage"] = sources
            state.pop("codex_transcript_cursor", None)
            state.pop("turn_id", None)
            state.pop("turn_off_record", None)
            state.pop("spooled_active", None)
            remaining = [
                item for item in state.get("pending_context_observations", [])
                if item not in observations
            ]
            if remaining:
                state["pending_context_observations"] = remaining
            else:
                state.pop("pending_context_observations", None)
        # If this fails, the caller must not start the network request.
        write_state(path, state)
        return deepcopy(staged) if staged and _commit_payload_has_response(staged.get("payload")) else None
...
def stop() -> None:
    ...
    try:
        external = external_session_id(payload)
        if external is None:
            raise ValueError("Stop has no valid native session identity")
        root = project_root(payload)
        project = load_project(root)
        binding = _binding_for_project(root, project)
        path, _ = load_bound_state(str(project["id"]), client, external, binding.binding_id)
        staged = _stage_stop(
            path, payload, project_id=str(project["id"]), client=client,
            external_session=external, binding_id=binding.binding_id,
        )
        transport = _project_client(binding, "hook", external)
        if staged is not None:
            try:
                response = _request(
                    transport, "POST", f"/turns/{staged['turn_id']}/stop-check",
                    json=staged["payload"], timeout=_stop_timeout(),
                )
                if _committed_stop_response(response):
                    _ack_pending_commit(path, staged)
            except Exception:
                pass  # The exact completion is durably queued.
        # Stop delivers only its own snapshot. Deferred usage sources have
        # additional recovery opportunities in SessionStart and Prompt.
        try:
            deadline = time.monotonic() + 2
            state = read_state(path)
            before = deepcopy(state)
            _, blocked = drain_pending_codex_usage(
                state, project_id=str(project["id"]), deadline=deadline, max_sources=1,
            )
            flush_client_telemetry(transport, str(project["id"]), deadline=deadline)
            if blocked and time.monotonic() < deadline:
                drain_pending_codex_usage(
                    state, project_id=str(project["id"]), deadline=deadline, max_sources=1,
                )
            merge_state_update(path, before, state, lock_timeout=1)
        ...
        print("dDuo: Stop capture could not be verified; queued data is retained.", file=sys.stderr)
