...
def start_local_setup(
    ...
    *,
    ...
    reuse_key_from_project: str | None = None,
    use_new_key: bool = False,
) -> dict:
    """After activation consent, reuse ready prerequisites before opening a page.

    The first call can return a key choice without creating anything. Keys
    stay on the host; the agent passes only a selected registered project ID.
    Existing active projects retain the explicit repair-page workflow.
    """
    from dduo_solo_founder.local_setup import (
        embedded_key_choices,
        prepare_local_project,
        reuse_embeddings_key,
    )

    if reuse_key_from_project and use_new_key:
        raise ValueError("Choose either an existing embeddings key or a new one.")
    root = find_workspace_root(project_root)
    had_config = (root / CONFIG_PATH).exists()
    status = get_setup_status(root)
    if not (status.get("project") or {}).get("ready"):
        if not (status.get("embeddings") or {}).get("ready"):
            choices = embedded_key_choices(root)
            if choices and not reuse_key_from_project and not use_new_key:
                return {"opened": False, "requires_key_choice": True, "key_choices": choices}
        project = prepare_local_project(root)
        allow_setup(root)
        if reuse_key_from_project:
            reuse_embeddings_key(root, reuse_key_from_project)
        if not had_config:
            if provider == "claude":
                # Let the official client check its native login. This creates
                # only private settings; it does not copy account credentials.
                ensure_project_claude_config_dir(str(project["id"]))
            # Only first-time onboarding may import the selected interactive
            # Codex profile. Never borrow another project's account or replace
            # an existing project login; the helper preserves existing auth.
            status = get_setup_status(root)
            if not any(item.get("ready") for item in status.get("clients", {}).values()):
                try:
                    installation = resolve_client_installation("codex")
                    ensure_project_codex_home(
                        str(project["id"]), import_global_auth=True,
                        source_codex_home=installation.config_dir,
                    )
                except (ClientInstallationError, OSError, ValueError):
                    pass  # A missing usable login is handled by native Setup.
                status = get_setup_status(root)
        if not use_new_key and embeddings_configured(str(project["id"])):
            hooks_ready = provider != "codex" or (status.get("codex_hooks") or {}).get("ready")
            if (
                (status.get("docker") or {}).get("ready")
                and hooks_ready
                and any(item.get("ready") for item in status.get("clients", {}).values())
            ):
                try:
                    result = subprocess.run(
                        [sys.executable, "-m", "dduo_solo_founder.launcher", "init",
                         "--yes", "--project-root", str(root)],
                        capture_output=True, text=True, timeout=210, check=False,
                    )
                except (OSError, subprocess.TimeoutExpired):
                    result = None
                if result is not None and result.returncode == 0:
                    return {"opened": False, "status": get_setup_status(root)}
                # Do not expose subprocess output, which may contain host data.
                open_setup(root)
                return {"opened": True, "activation_failed": True}
    open_setup(root)
    return {"opened": True}
...
def configure_openai(
    ...
    with embedding_key_lock(str(project["id"])):
...
def login_codex(
    ...
    surface: str = typer.Option("unknown", "--surface"),
    client_config_dir: Path | None = typer.Option(None, "--client-config-dir"),
    client_executable: Path | None = typer.Option(None, "--client-executable"),
    client_probe_executable: Path | None = typer.Option(None, "--client-probe-executable"),
) -> None:
    ...
    try:
        installation = resolve_client_installation(
            ...
            probe_executable=client_probe_executable,
        )
        executable = resolve_codex_executable(required=True, installation=installation)
    except (ClientInstallationError, FileNotFoundError) as exc:
        raise typer.BadParameter(str(exc)) from exc
    project_id = str(project["id"])
    ensure_project_codex_home(
        project_id, import_global_auth=True, source_codex_home=installation.config_dir
    )
    ...
    status = subscription_auth_status(
        "codex", environment=environment, installation=installation
    )
...
def login_claude(
    ...
    """Authenticate Claude subscription sleep for this project's private host state."""
    root = find_workspace_root(project_root)
    project = load_project(root)
    if _runtime_binding(root, project).remote:
        raise typer.BadParameter("authenticate on the memory host, not a remote-bound checkout")
    ...
    environment = claude_environment(project_id, detached_cli_environment())
    result = subprocess.run(
        client_command(
    ...
    if result.returncode:
        raise typer.Exit(result.returncode)
    status = subscription_auth_status(
        "claude", environment=environment, installation=installation
    )
    typer.echo(json.dumps({"ready": status.ready, "provider": "claude"}))
    if not status.ready:
        raise typer.Exit(8)
...
def client_readiness_command(
    ...
    client_config_dir: Path | None = typer.Option(None, "--client-config-dir"),
    client_executable: Path | None = typer.Option(None, "--client-executable"),
    client_node_executable: Path | None = typer.Option(None, "--client-node-executable"),
    client_probe_executable: Path | None = typer.Option(None, "--client-probe-executable"),
    repair_client_record: bool = typer.Option(False, "--repair-client-record"),
) -> None:
    ...
    try:
        installation = resolve_client_installation(
            ...
            config_dir=client_config_dir,
            executable=client_executable,
            node_executable=client_node_executable,
            probe_executable=client_probe_executable,
            repair=repair_client_record,
        )
        result = inspect_client_readiness(client, project_root, installation=installation)
    except (ValueError, ClientInstallationError) as exc:
...
def init_project(
    ...
    if not embeddings_configured(project_id):
        ...
        typer.echo(json.dumps({"setup_required": True, "project_id": project_id,
                              "next_action": "configure-openai, login-codex --device-auth or login-claude, then rerun init --headless"
                              if headless else "complete_setup"}))
        raise typer.Exit(5)
    if headless:
        _require_remote_sleep_auth(str(project_id))
...
def _require_remote_sleep_auth(project_id: str) -> str:
    ...
    A project has one shared queue.  The live bridge will later prefer the
    subscription selected by the project's first supported chat and use this
    other verified provider only when the preference is unavailable.
    """
    checks: dict[str, object] = {}
    for provider in ("codex", "claude"):
        try:
            installation = resolve_client_installation(provider)
            environment = (
        ...
        except (OSError, ValueError, ClientInstallationError):
            continue
        checks[provider] = status
        if status.ready:
            return provider
    reasons = ", ".join(
        f"{provider}: {getattr(status, 'reason', 'unavailable')}"
        for provider, status in checks.items()
    ) or "no supported CLI is available"
    raise RuntimeError(
        "A Codex or Claude subscription login is required on this VPS before remote hosting. "
        "Run `dduo-solo-founder login-codex --device-auth --project-root <server-checkout>` "
        "or `dduo-solo-founder login-claude --project-root <server-checkout>`, then retry remote-host "
        f"({reasons})."
    )
...
def remote_host(
    ...
    _require_remote_sleep_auth(project["id"])
...
def _restore_full_recovery_supplement(
    ...
    *,
    ...
    claude_source = extracted / "secrets/claude/.credentials.json"
    if claude_source.is_file():
        claude_auth = read_json(claude_source, "Claude .credentials.json")
        if not isinstance(claude_auth, dict):
            raise BackupError("full recovery Claude .credentials.json is malformed")
        auth_target = project_claude_config_dir(project_id) / ".credentials.json"
        if force or not auth_target.exists():
            prepared_writes.append((auth_target, json_bytes(claude_auth)))
        claude_restored = True

    hook_count = 0
    hooks = extracted / "host-state/hooks"
    if hooks.is_dir():
        # The private outbox filename is scoped to the current binding. A
        # restore onto a new checkout/endpoint must therefore recalculate it
        # instead of reviving a stale filename or a previous live-context
        # baseline.
        from dduo_solo_founder.hooks import state_path
        ...
        for source in sorted(hooks.glob("*.json")):
            ...
            external_session_id = state.get("session_external_id")
            if not isinstance(client, str) or not isinstance(external_session_id, str):
                raise BackupError("full recovery hook state lacks a bound client session")
            client = client.strip().lower()
            external_session_id = external_session_id.strip()
            filename_client = next(
                (family for family in ("codex", "claude") if source.name.startswith(f"{project_id}-{family}-")),
                None,
            )
            if not client and filename_client and external_session_id:
                # Older ledgers recorded the family in the deterministic
                # filename, not in JSON. Infer only that exact legacy identity.
                if source.name != state_path(project_id, filename_client, external_session_id).name:
                    raise BackupError("full recovery legacy hook filename does not match its session")
                client = filename_client
            if client not in {"codex", "claude"} or not external_session_id:
                raise BackupError("full recovery hook state lacks a bound client session")
            if filename_client != client:
                raise BackupError("full recovery hook client does not match its filename")
            archived_binding_id = state.get("binding_id")
            if archived_binding_id is not None and (
                not isinstance(archived_binding_id, str) or not archived_binding_id
            ):
                raise BackupError("full recovery hook binding is malformed")
            # The archive belongs to this project, but a nested item carrying
            # another binding must not acquire authority through restoration.
            # Legacy items without a binding inherit their validated ledger.
            for queue_key in ("pending_commits", "spooled_turns"):
                queue = state.get(queue_key, [])
                if not isinstance(queue, list):
                    raise BackupError("full recovery hook queue is malformed")
                for item in queue:
                    if not isinstance(item, dict) or item.get("binding_id") not in (None, archived_binding_id):
                        raise BackupError("full recovery hook queue belongs to another binding")
                    item["binding_id"] = binding.binding_id
            active = state.get("spooled_active")
            if active is not None:
                if not isinstance(active, dict) or active.get("binding_id") not in (None, archived_binding_id):
                    raise BackupError("full recovery active hook turn belongs to another binding")
                active["binding_id"] = binding.binding_id
            state["project_id"] = project_id
            state["client"] = client
            state["binding_id"] = binding.binding_id
            # A restored host has not demonstrated delivery to a live chat.
            # Keep durable retry queues and privacy, but force the next
            # SessionStart to establish a fresh baseline and evidence.
            state.pop("context_delivery_baseline", None)
            state.pop("delivery_baseline", None)
            state.pop("last_briefing_context_hash", None)
            state.pop("lifecycle_evidence", None)
            target = state_path(
                project_id,
                client,
                external_session_id,
...
def restore_backup_command(
    ...
    typer.echo(
        ...
        f"Claude auth restored: {str(bool(supplement_result.get('claude_auth'))).lower()}, "
