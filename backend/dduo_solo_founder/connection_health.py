...
def sleep_connection_notice(status: dict, *, remote: bool = False) -> dict | None:
    ...
    if auth:
        ...
        message = (
            "Memory consolidation is paused: no verified Codex or Claude subscription is available "
            f"for the project's {(provider or 'preferred').title()} sleep preference. Saved turns and existing memories are retained. "
        ...
        action = (
            ...
            if remote else
            "Offer to open this project's configuration now to connect either Codex or Claude for memory sleep. "
            "After the user chooses repair, call open_setup and guide them to select Connect. "
            "Complete one official subscription sign-in on the memory host, then recheck. "
