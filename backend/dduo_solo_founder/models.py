...
class Project(Base):
    ...
    # The first supported client to open a newly activated project chooses the
    # preferred sleep subscription.  The bridge can still use the other
    # provider when the preferred one is unavailable on the memory host.
    sleep_executor_preference: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "sleep_executor_preference IS NULL OR sleep_executor_preference IN ('codex', 'claude')",
            name="ck_projects_sleep_executor_preference",
        ),
    )
...
class SleepJob(Base):
    ...
    session_id: Mapped[str] = mapped_column(ForeignKey("agent_sessions.id", ondelete="CASCADE"))
    segment_id: Mapped[str | None] = mapped_column(ForeignKey("segments.id"), nullable=True)
    # Stable requested order for this job.  ``provider`` records the provider
    # that actually ran the most recent attempt, including a safe fallback.
    executor_preference: Mapped[str | None] = mapped_column(String(20), nullable=True)
