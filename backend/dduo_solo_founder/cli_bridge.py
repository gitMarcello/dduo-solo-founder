...
class CliRunner:
    ...
    @staticmethod
    def _installation(provider: str) -> ClientInstallation:
        try:
            return resolve_client_installation(provider)
        except ClientInstallationError as exc:
            raise CliExecutionError("dependency_unavailable", str(exc)) from exc

    @staticmethod
    def _provider_environment(provider: str, project_id: str) -> dict[str, str]:
        """Return the host-owned, project-isolated environment for one CLI.

        The active interactive chat never supplies credentials here.  On a
        VPS this code runs on the VPS; on a local project it runs on that
        project owner's computer.
        """
        base = detached_cli_environment()
        if provider == "codex":
            return codex_environment(project_id, base)
        return claude_environment(project_id, base)

    def _eligible_provider_order(
        self, preferred: str, project_id: str
    ) -> list[tuple[ClientInstallation, dict[str, str]]]:
        """Return authenticated host subscriptions, preferred provider first."""
        candidates = [preferred, *(item for item in ("codex", "claude") if item != preferred)]
        eligible: list[tuple[ClientInstallation, dict[str, str]]] = []
        for candidate in candidates:
            try:
                installation = self._installation(candidate)
                environment = self._provider_environment(candidate, project_id)
                status = subscription_auth_status(
                    candidate,
                    environment=environment,
                    installation=installation,
                )
            except (OSError, ValueError, CliExecutionError):
                continue
            if status.ready:
                eligible.append((installation, environment))
        return eligible

    @staticmethod
    def _fallback_eligible(error: CliExecutionError) -> bool:
        """Retry only an unavailable login or executable before any model output.

        A rate limit is not an availability signal: silently spending a second
        subscription to bypass it would surprise the infrastructure manager.
        """
        return error.kind in {"auth_required", "dependency_unavailable"}
    ...
    def _run_provider(
        ...
        *,
        ...
        if installation is not None:
            run = self._run_codex if provider == "codex" else self._run_claude
            return run(
                prompt, schema, timeout, project_id,
                installation=installation, environment=environment,
            )
        if provider == "codex":
            # Keep the historical three-argument call for direct bridge tests.
            return (
        ...
        return (
            self._run_claude(prompt, schema, timeout, project_id)
            if project_id
            else self._run_claude(prompt, schema, timeout)
        )

    def generate(self, request: dict[str, Any]) -> dict[str, Any]:
        preferred_provider = str(request.get("provider") or "").strip().lower()
        ...
        if preferred_provider not in {"codex", "claude"}:
        ...
        try:
            with self._lock:
                # Legacy direct bridge callers have no project identity.  They
                # retain their explicit provider behaviour; dDuo sleep jobs
                # always send a project id and therefore use host resolution.
                candidates = (
                    self._eligible_provider_order(preferred_provider, project_id)
                    if project_id
                    else [(None, None)]
                )
                if not candidates:
                    raise CliExecutionError(
                        "auth_required",
                        "Connect either Codex or Claude on the computer running memory, then retry.",
                        provider=preferred_provider,
                    )
                last_error: CliExecutionError | None = None
                for index, (installation, environment) in enumerate(candidates):
                    provider = installation.family if installation is not None else preferred_provider
                    try:
                        result = self._run_provider(
                            provider, prompt, schema, timeout, project_id,
                            installation=installation, environment=environment,
                        )
                        break
                        ...
                        last_error = exc
                        if index + 1 < len(candidates) and self._fallback_eligible(exc):
                            continue
                        raise
                else:  # pragma: no cover - defensive; the loop either breaks or raises.
                    raise last_error or CliExecutionError(
                        "bridge_unavailable", "No memory subscription could run."
                    )
                if len(result) == 2:
        ...
        response = {
            "ok": True,
            "provider": provider,
            "executor_preference": preferred_provider,
            "duration_seconds": duration_seconds,
            "output": parsed,
        ...
        if provider != preferred_provider:
            response["fallback_from"] = preferred_provider
        return response
    ...
    def _run_codex(
        ...
        *,
        ...
        try:
            installation = installation or CliRunner._installation("codex")
            executable = resolve_codex_executable(installation=installation)
        except (ClientInstallationError, CliExecutionError):
            executable = None
        if not executable:
            raise CliExecutionError(
            ...
            try:
                process = subprocess.run(
                    ...
                    env=(
                        ...
                        else codex_environment(project_id, detached_cli_environment())
                        if project_id
    ...
    def _run_claude(
        ...
        *,
        ...
        try:
            installation = installation or CliRunner._installation("claude")
            executable = str(installation.management_command.executable)
        except CliExecutionError:
            executable = None
            installation = None
        if not executable or installation is None:
            raise CliExecutionError(
        ...
        with tempfile.TemporaryDirectory(prefix="dduo-claude-sleep-") as temporary:
            try:
                process = subprocess.run(
                    ...
                    env=(
                        environment if environment is not None
                        else claude_environment(project_id, detached_cli_environment())
                        if project_id
                        else detached_cli_environment()
                    ),
                )
...
class SetupService:
    ...
    def __init__(self):
        ...
        self._hook_status: tuple[float, tuple, Any] | None = None
        self._lock = threading.RLock()
    ...
    def _subscription_status(
        ...
        *,
        ...
        now = time.monotonic()
        try:
            installation = CliRunner._installation(provider)
        except CliExecutionError:
            installation = None
        # A readiness result is meaningful only for the precise command and
        # management configuration that were checked.  In particular, do not
        # reuse a result after an explicit repair changes either path.
        installation_identity = (
            ...
            if installation is None
                ...
                (
                    ...
                    str(installation.management_command.node_executable or ""),
                    str(installation.config_dir),
                    installation.surface_hint,
                )
            )
        )
        cache_key = f"{provider}:{project_id or 'host'}:{installation_identity}"
        with self._lock:
            ...
            with CLI_PROCESS_LOCK:
                ...
                elif provider == "claude" and project_id:
                    config_dir = project_claude_config_dir(project_id)
                    if not config_dir.is_dir():
                        return SubscriptionAuthStatus(
                            provider, False, "login_required", "claude auth login --claudeai"
                        )
                    environment = {**detached_cli_environment(), "CLAUDE_CONFIG_DIR": str(config_dir)}
                try:
                    if installation is None:
                        raise CliExecutionError(
                            "dependency_unavailable",
                    ...
                    status = subscription_auth_status(
                        provider, environment=environment, installation=installation
                    )
                except CliExecutionError:
                    status = SubscriptionAuthStatus(
                        ...
                        False,
                        "cli_missing",
                        "codex login" if provider == "codex" else "claude auth login --claudeai",
                    )
    ...
    def _codex_hook_status(self, root: Path | None) -> Any:
        if root is None or not root.exists():
            return None
        try:
            installation = CliRunner._installation("codex")
        except CliExecutionError:
            installation = None
        key = (str(root.resolve()), installation)
        now = time.monotonic()
        with self._lock:
    ...
    def _project_status(root: Path | None) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                # The descriptor name is only a bootstrap label. Older servers
                # may not expose identity yet; their health remains independent.
                try:
                    binding = binding_from_project(root, project)
                    identity_response = httpx.get(
                        f"{binding.api_url}/projects/{binding.project_id}/identity",
                        headers=ProjectHttpClient(binding, component="setup").headers(),
                        timeout=1, trust_env=False, follow_redirects=False,
                    )
                    identity = identity_response.json() if identity_response.status_code == 200 else {}
                    if (isinstance(identity, dict) and identity.get("id") == project["id"]
                            and isinstance(identity.get("name"), str) and identity["name"].strip()):
                        name = identity["name"]
    ...
    def status(self, project_root: str | None = None) -> dict[str, Any]:
        ...
        for provider in ("claude", "codex"):
            ...
            with self._lock:
                ...
                client["resume_ready"] = bool(
                    attempt and attempt.verified and not attempt.resume_attempted
                )
                client["resume_failed"] = bool(attempt and attempt.resume_failed)
            # A freshly verified native login is sufficient to resume the
            # auth-paused job.  Do not let the stale server status turn that
            # just-completed setup into another apparent reconnection loop.
            if (
                self._provider_auth_required(memory_status, provider)
                and setup_state != "connected"
            ):
                client.update(ready=False, reason="auth_required")
            elif setup_state in {"waiting", "checking", "failed"}:
                client["ready"] = False
            clients[provider] = client
        # Submit at most one automatic resume.  Two connected subscriptions
        # must not let one Setup refresh post the same paused sleep job twice.
        # Prefer the project's chosen executor, then the other ready host
        # subscription; the bridge still verifies that choice at execution.
        preferred_provider = memory_status.get("executor_provider")
        provider_order = [
            preferred_provider,
            *(item for item in ("codex", "claude") if item != preferred_provider),
        ]
        resume_candidates = [
            provider
            for provider in provider_order
            if provider in clients and clients[provider].get("resume_ready")
        ]
        if memory_status.get("state") == "connection_required":
            for client in clients.values():
                client["resume_ready"] = False
            for provider in provider_order:
                if (
                    provider in clients
                    and clients[provider].get("ready")
                    and not clients[provider].get("resume_failed")
                ):
                    resume_candidates = [provider]
                    break
        if resume_candidates:
            for client in clients.values():
                client["resume_ready"] = False
            clients[resume_candidates[0]]["resume_ready"] = True
        ...
        # Capture and sleep are deliberately independent.  A Codex chat can
        # record turns while the project host consolidates through Claude (or
        # the reverse).  Requiring the interactive client's *own* subscription
        # here incorrectly marks that fully working arrangement disconnected.
        memory_ready = memory_status.get("state") in {"updated", "updating"}
        integration_ready = {
            "claude": True,
            "codex": bool((codex_hooks or {}).get("ready")),
        }
        return {
            "docker": docker,
            ...
            "ready_for_client": {
                "claude": (
                    infrastructure_ready
                    and memory_ready
                    and integration_ready["claude"]
                ),
                "codex": (
                    infrastructure_ready
                    and memory_ready
                    and integration_ready["codex"]
                ),
            },
        }
    ...
    def save_openai_key(self, key: str, project_root: str | None = None) -> None:
        value = key.strip()
        if len(value) < 8 or any(char in key for char in "\x00\r\n"):
            raise ValueError("Enter a valid OpenAI embeddings key.")
        if project_root and not (Path(project_root) / ".dduo-solo-founder/project.toml").exists():
            # This is an explicit, root-bound POST, not the polling GET. Prepare
            # identity before saving the key; starting Docker remains separate.
            from dduo_solo_founder.local_setup import prepare_local_project

            prepare_local_project(Path(project_root))
        ...
        if selected and str(selected[1].get("binding") or "local").lower() != "local":
            raise ValueError("Configure embeddings on the memory host, not this remote checkout.")
        from dduo_solo_founder.local_setup import embedding_key_lock

        with embedding_key_lock(project_id):
    ...
    def start_auth(self, provider: str, project_root: str | None = None) -> dict[str, Any]:
        ...
        try:
            installation = CliRunner._installation(provider)
            executable = (
                resolve_codex_executable(installation=installation)
                if provider == "codex"
                else str(installation.management_command.executable)
            )
        except CliExecutionError:
            executable = None
            installation = None
        ...
        with self._lock:
            ...
            if existing and existing.process and existing.process.poll() is None:
                return {"status": "waiting", "provider": provider}
            try:
                environment = installation.environment(detached_cli_environment())
                if provider == "codex" and project_id:
                    ...
                    environment["CLAUDE_CONFIG_DIR"] = str(
                        ensure_project_claude_config_dir(project_id)
                    )
            ...
            # Readiness is keyed by provider, project, executable, private
            # configuration and surface. A new native login can make every
            # cached outcome for that provider/project obsolete; removing the
            # old short prefix would leave the actual keyed entry behind.
            cache_prefix = f"{provider}:{project_id or 'host'}:"
            for cache_key in tuple(self._auth_status):
                if cache_key.startswith(cache_prefix):
                    self._auth_status.pop(cache_key, None)
        return {"status": "waiting", "provider": provider}
    ...
    def resume_memory(self, project_root: str, provider: str) -> dict[str, Any]:
        """Retry auth-paused jobs through any verified host subscription."""
        root = Path(project_root).expanduser().resolve()
        selected = SetupService._project_config(root)
        ...
        if self._memory_status(root).get("state") != "connection_required":
            return {"resumed": False, "provider": provider}
        with self._lock:
            attempt = self._auth.get(f"{provider}:{project_id}")
            status = self._subscription_status(provider, project_id=project_id, fresh=True)
            if not status.ready:
                return {"resumed": False, "provider": provider}
            if attempt is None:
                # An already-authenticated alternate provider can unblock a
                # job that paused before it was ever connected through Setup.
                # Keep this one resume attempt in the same local state machine
                # so a page refresh cannot submit the sleep job repeatedly.
                attempt = AuthAttempt(
                    provider=provider,
                    started_at=time.monotonic(),
                    verified=True,
                )
                self._auth[f"{provider}:{project_id}"] = attempt
            if attempt and self._verify_auth_attempt(provider, project_id) != "connected":
                return {"resumed": False, "provider": provider}
            if attempt and (attempt.resume_in_flight or (attempt.resume_attempted and not attempt.resume_failed)):
                return {"resumed": False, "provider": provider}
            if attempt:
            ...
            <script>
              ...
              const COPY = {
                en: {
                  ...
                  eyebrow: 'Local setup',
                  intro: 'Connect OpenAI for embeddings and one supported subscription for sleep. dDuo checks everything else automatically.',
                  systemTitle: 'System status',
                  systemDescription: 'One of Codex or Claude is enough for sleep. The project prefers the client that started it, then uses the other verified host subscription only when needed.',
                  connected: 'Connected',
                  waiting: 'Waiting',
                  ...
                  clientReady: 'Saved subscription sign-in is available for memory sleep.',
                  clientReconnect: 'No other supported subscription is available. Reconnect in the official window to resume memory sleep.',
                  ...
                  clientIdle: 'Connect this client if you want it to run memory sleep. One connected client is enough.',
                  clientOptional: 'Optional fallback. Another supported subscription is already ready for memory sleep.',
                  connect: 'Connect',
                  reconnect: 'Reconnect',
                  optional: 'Not needed',
                  ...
                  memoryConnection: 'Consolidation is paused until one supported subscription is connected on this memory host.',
                ...
                it: {
                  ...
                  intro: 'Collega OpenAI per gli embedding e un solo abbonamento supportato per il sonno. dDuo verifica automaticamente tutto il resto.',
                  systemTitle: 'Stato del sistema',
                  systemDescription: 'Per il sonno basta Codex o Claude. Il progetto preferisce il client che lo ha avviato e usa l’altro abbonamento verificato sull’host solo quando necessario.',
                  connected: 'Connesso',
                  waiting: 'In attesa',
                  ...
                  clientReady: 'È disponibile un accesso salvato all’abbonamento per il sonno della memoria.',
                  clientReconnect: 'Non è disponibile un altro abbonamento supportato. Ricollega nella finestra ufficiale per riprendere il sonno della memoria.',
                  ...
                  clientIdle: 'Connetti questo client se vuoi che esegua il sonno della memoria. Basta un solo client collegato.',
                  clientOptional: 'Fallback opzionale. Un altro abbonamento supportato è già pronto per il sonno della memoria.',
                  connect: 'Connetti',
                  reconnect: 'Ricollega',
                  optional: 'Non necessario',
                  ...
                  memoryConnection: 'Il consolidamento è sospeso finché non colleghi un abbonamento supportato su questo host della memoria.',
              ...
              function setupErrorMessage(value) {
                ...
                const keys = {
                  ...
                  'Initialize this project before saving the embeddings key.': 'errorProjectConfiguration',
                  'Initialize this project before connecting a client.': 'errorProjectConfiguration',
                  'Configure embeddings on the memory host, not this remote checkout.': 'errorProjectConfiguration',
              ...
              function renderSetupState(state) {
                ...
                const sleepProviderReady = ['claude', 'codex'].some(
                  (provider) => Boolean(state.clients?.[provider]?.ready),
                );
                for (const provider of ['claude', 'codex']) {
                  const client = state.clients[provider];
                  const optionalFallback = sleepProviderReady
                    && !client.ready
                    && !client.resume_failed
                    && client.setup_state !== 'waiting'
                    && client.setup_state !== 'failed';
                  renderRow(
                    provider,
                    provider === 'claude' ? 'Claude' : 'Codex',
                    optionalFallback ? t('clientOptional') : client.resume_failed ? t('memoryResumeFailed') : client.setup_state === 'waiting'
                        ? t('clientWaiting')
                        : client.setup_state === 'failed'
                          ? t('clientFailed')
                    ...
                    t(optionalFallback ? 'optional' : client.resume_failed ? 'memoryRetry' : client.reason === 'auth_required' ? 'reconnect' : 'connect'),
                    optionalFallback ? null : () => client.resume_failed ? resumeMemory(provider) : connect(provider),
                  );
...
def build_backup_supplement(project_id: str) -> dict[str, Any]:
    ...
    # Claude Code stores file-backed subscription state in the configured
    # directory on Linux and Windows. macOS may put it in Keychain instead;
    # dDuo never reads the OS keychain, so a project can still be recovered
    # safely but may need one explicit Claude login afterwards on that host.
    claude_credentials_path = project_claude_config_dir(project_id) / ".credentials.json"
    if claude_credentials_path.is_file():
        include("secrets/claude/.credentials.json", claude_credentials_path)
        claude_auth = {"included": True, "credential_store": "file"}
    else:
        claude_auth = {"included": False, "credential_store": "unavailable"}

    # A project needs one host-owned subscription executor, not both. A
    # missing credential is therefore only a recovery warning if neither
    # supported client has portable file-backed state.
    if not codex_auth["included"] and not claude_auth["included"]:
        warnings.append("sleep_auth_unavailable")
