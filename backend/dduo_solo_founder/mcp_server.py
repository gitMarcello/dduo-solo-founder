...
from dduo_solo_founder.launcher import start_local_setup
...
class CheckMemoryConnectionInput(BaseModel):
    """Optionally correlate readiness with the session in delivered context."""

    dduo_session_id: uuid.UUID | None = None
    dduo_turn_id: uuid.UUID | None = None
...
class OpenSetupInput(BaseModel):
    reuse_key_from_project: uuid.UUID | None = Field(
        default=None,
        description="Project ID returned in key_choices; use only after the user explicitly selects reusing its embeddings key. Never pass the key itself.",
    )
    use_new_key: bool = Field(
        default=False,
        description="The user chose a different embeddings key; open the protected input page. Mutually exclusive with reuse_key_from_project.",
    )
...
TOOL_MODELS = {
    ...
    "open_setup": (
        "After activation consent, offer existing embeddings-key reuse and activate with ready local prerequisites; open Setup only for missing actions or repair. Remote bindings never start Docker.",
        OpenSetupInput,
    ),
    "check_setup": (
    ...
    "rename_project": (
        "Rename this project's display name only on explicit user request. Infrastructure manager only. "
        "Use profile_version from the latest briefing as expected_version. Repository, UUID and data stay unchanged.",
        ProjectRename,
    ),
}
...
def _setup_check(status: dict, provider: str | None) -> dict:
    ...
    # Sleep resolves an authenticated host subscription at execution time. Do
    # not send a founder through a second sign-in merely because the preferred
    # provider is unavailable while the other supported provider is ready.
    available_sleep_provider = next(
        (
            name
            for name in (executor, "codex", "claude")
            if name in {"codex", "claude"}
            and bool((clients.get(name) or {}).get("ready"))
        ),
        None,
    )
    login_provider = (
        None
        if available_sleep_provider
        else executor
    ...
    if login_provider:
        client = clients.get(login_provider) or {}
    ...
    if (
        issue is not None
        and not available_sleep_provider
        and bool((status.get("project") or {}).get("ready"))
    ):
...
def call(
    ...
    if name == "health":
        ...
        if not project_id:
            ...
            return {
                "status": "unconfigured",
                "instruction": (
                    "Briefly offer to activate this project with memory and organized Work, "
                    "with continuing without memory as the explicit alternative. After consent, use open_setup; "
                    "follow its key-choice or readiness result, without promising a configuration page. "
    ...
    if name == "open_setup":
        ...
        if project_root is None:
            raise ValueError("the client adapter did not supply a project root")
        root = project_root
        result = start_local_setup(
            root,
            provider=current_setup_provider(),
            reuse_key_from_project=(
                str(args["reuse_key_from_project"]) if args.get("reuse_key_from_project") else None
            ),
            use_new_key=args.get("use_new_key", False),
        )
        if result.get("requires_key_choice"):
            return {
                **result,
                "response_instruction": (
                    "Ask one short question in the user's language: Would you like to reuse the OpenAI key "
                    "already configured on this computer, or enter a different one? If there are multiple "
                    "choices, identify them by project name, never by IDs or credentials. Wait for the answer. "
                    "On explicit reuse call open_setup with the selected key_choices.project_id as "
                    "reuse_key_from_project; for a different key set use_new_key=true. Do not open a page yet."
                ),
            }
        if not result.get("opened"):
            return {"opened": False, **_setup_check(result["status"], current_setup_provider())}
        return {
            "opened": True,
            "activation_failed": bool(result.get("activation_failed")),
            "response_instruction": (
                ("Automatic activation did not complete; do not claim success. " if result.get("activation_failed") else "")
    ...
    if name == "rename_project":
        result = request(api, "POST", f"/projects/{project_id}/rename", args)
        return {
            "name": result["name"],
            "profile_version": result["profile_version"],
            "response_instruction": (
                "Confirm the new project name in the user's language. "
                "Repository, memory and links are unchanged."
            ),
        }
...
def _empty_lifecycle(*, correlation: str = "unknown") -> dict:
    return {
        "correlation": correlation,
        "session_start_observed": None,
        "prompt_observed": None,
        "context_emitted": None,
        "completion": "unknown",
    }


def _lifecycle_from_state(state: dict, turn_id: str | None) -> dict:
    """Render content-free, locally observed evidence for one exact state file."""
    evidence = state.get("lifecycle_evidence")
    evidence = evidence if isinstance(evidence, dict) and evidence.get("version") == 1 else {}
    session_start = evidence.get("session_start")
    prompt = evidence.get("prompt")
    stop = evidence.get("stop")
    # A session may have an old completed turn and a new open one. Without an
    # exact turn id, its latest Stop cannot prove capture of the current turn.
    prompt_matches = bool(turn_id) and isinstance(prompt, dict) and (
        str(prompt.get("turn_id") or "") == turn_id
    )
    stop_matches = bool(turn_id) and isinstance(stop, dict) and (
        str(stop.get("turn_id") or "") == turn_id
    )
    completion = "unknown"
    if stop_matches:
        if stop.get("off_record") is True:
            completion = "off_record"
        elif stop.get("server_committed_at"):
            completion = "persisted"
        elif stop.get("response_available") is False:
            completion = "incomplete"
        elif stop.get("response_available") is True and stop.get("queued_at"):
            completion = "queued"
    elif turn_id is not None and str(state.get("turn_id") or "") == turn_id:
        completion = "off_record" if state.get("turn_off_record") is True else "not_yet_completed"
    if completion == "unknown" and turn_id:
        pending = state.get("pending_commits")
        if isinstance(pending, list) and any(
            isinstance(item, dict) and str(item.get("turn_id") or "") == turn_id
            and isinstance(item.get("payload"), dict)
            and isinstance(item["payload"].get("assistant_response"), str)
            for item in pending
        ):
            completion = "queued"
    return {
        ...
        "session_start_observed": (
            True if isinstance(session_start, dict) and session_start.get("observed_at") else None
        ),
        "prompt_observed": True if prompt_matches and prompt.get("observed_at") else None,
        "context_emitted": (
            True if prompt_matches and prompt.get("context_emitted_at") else None
        ),
        "completion": completion,
    }


def _local_lifecycle_state(
    runtime: MCPToolRuntime, dduo_session_id: str, external_session_id: str | None = None
) -> tuple[str, dict | None]:
    """Find only an exact state match; never substitute a recent chat."""
    if not runtime.project_id or runtime.binding is None:
        return "unknown", None
    from dduo_solo_founder.hooks import HOOK_STATE_DIR, read_state, state_path
    ...
    if external_session_id:
        ...
        # Offline diagnostics have only the internal session id. Search the
        # binding-qualified ledger and require exactly one matching value.
        for path in HOOK_STATE_DIR.glob(f"{runtime.project_id}-{runtime.client}-*.json"):
            candidate = read_state(path)
            if (
                candidate.get("project_id") == runtime.project_id
                and candidate.get("binding_id") == runtime.binding.binding_id
                and str(candidate.get("session_id") or "") == dduo_session_id
            ):
                candidates.append(candidate)
    if len(candidates) == 1:
        ...
        if (
            candidate.get("project_id") == runtime.project_id
            and candidate.get("binding_id") == runtime.binding.binding_id
            and candidate.get("client") == runtime.client
            and str(candidate.get("session_id") or "") == dduo_session_id
        ):
            return "exact", candidate
    return ("ambiguous" if len(candidates) > 1 else "unknown"), None


def _capture_lifecycle(
    runtime: MCPToolRuntime, dduo_session_id: str | None, dduo_turn_id: str | None = None
) -> dict:
    """Read lifecycle state for the supplied session only; never guess one."""
    unknown = _empty_lifecycle()
    if not dduo_session_id or not runtime.project_id:
        return unknown
    try:
        params: dict[str, str] = {"client": runtime.client}
        if dduo_turn_id:
            params["turn_id"] = dduo_turn_id
        response = _raw_response(
            runtime.api,
            "GET",
            f"/projects/{runtime.project_id}/sessions/{dduo_session_id}/capture-status",
            params=params,
            timeout=3,
        )
        response.raise_for_status()
        ...
        # An older server without this endpoint is unverified, not disconnected.
        # Do not replace a server-denied identity with local evidence either.
        return unknown
    except (httpx.RequestError, ValueError, OSError):
        correlation, state = _local_lifecycle_state(runtime, dduo_session_id)
        return _lifecycle_from_state(state, dduo_turn_id) if state is not None else _empty_lifecycle(
            correlation=correlation
        )
    if not isinstance(payload, dict) or not isinstance(payload.get("session"), dict):
        return unknown
    session = payload["session"]
    if str(session.get("id") or "") != dduo_session_id or session.get("client") != runtime.client:
        return unknown
    correlation, state = _local_lifecycle_state(
        runtime,
        dduo_session_id,
        str(session.get("external_id") or "") or None,
    )
    local = _lifecycle_from_state(state, dduo_turn_id) if state is not None else _empty_lifecycle(
        correlation=correlation
    )
    turn = payload.get("turn")
    completion = local["completion"]
    prompt_observed = local["prompt_observed"]
    if dduo_turn_id and isinstance(turn, dict):
        if str(turn.get("id") or "") != dduo_turn_id:
            return unknown
        # Server turn existence proves an API turn, not that an IDE executed
        # UserPromptSubmit. Only the exact local hook evidence proves that.
        if turn.get("off_record") is True:
            completion = "off_record"
        elif turn.get("committed") is True:
            completion = "persisted"
        elif completion not in {"queued", "incomplete", "off_record"}:
            completion = "not_yet_completed" if turn.get("status") == "open" else "unknown"
    return {
        "correlation": "exact",
        "session_start_observed": local["session_start_observed"],
        "prompt_observed": prompt_observed,
        # The hook can prove only that it emitted an envelope, not that an IDE
        # accepted it or the model consumed it.
        "context_emitted": local["context_emitted"],
        "completion": completion,
    }
...
def _connection_notice(
    ...
    dduo_session_id: str | None = None,
    dduo_turn_id: str | None = None,
) -> dict | None:
    ...
    native = _MEMORY_CONNECTION_CHECKS.check(
    ...
    health = _read_connection_health(runtime)
    ...
    if name == "check_memory_connection":
        return {
            **checked,
            "integration": {
                "registered": native.get("registered"),
                "hooks": native.get("hooks"),
            },
            "lifecycle": _capture_lifecycle(runtime, dduo_session_id, dduo_turn_id),
            "sleep": {
                "state": health.get("state"),
                "provider": health.get("executor_provider") or health.get("provider"),
            },
        }
...
async def _call_mcp_tool(
    ...
    try:
        dduo_session_id = (
            str(arguments.pop("dduo_session_id"))
            if params.name == "check_memory_connection" and arguments.get("dduo_session_id")
            else None
        )
        dduo_turn_id = (
            str(arguments.pop("dduo_turn_id"))
            if params.name == "check_memory_connection" and arguments.get("dduo_turn_id")
            else None
        )
