...
def project_claude_config_dir(project_id: str) -> Path:
    """Return the private Claude configuration directory owned by one project.

    Claude Code honours ``CLAUDE_CONFIG_DIR``.  Keeping its file-backed state
    under the dDuo project boundary avoids borrowing another project's local
    configuration on Linux and Windows.  macOS may keep the actual login in
    Keychain; dDuo deliberately neither reads nor copies Keychain entries.
    """
    return project_secret_dir(project_id) / "claude"
...
def ensure_project_codex_home(
    ...
    *,
    ...
    containers. Import is an explicit onboarding-only convenience from the
    selected client profile, never an ambient or periodic credential lookup.
    OS keyring contents are deliberately never scraped.
    """
    if import_global_auth and (
        source_codex_home is None or not source_codex_home.is_absolute()
    ):
        raise ValueError("Codex auth import requires an explicit absolute source home")
    ...
    if import_global_auth and existing_auth is None:
        ...
        try:
            global_auth_content = _read_external_private_text(source_codex_home / "auth.json")
        except FileNotFoundError:
...
def ensure_project_claude_config_dir(project_id: str) -> Path:
    """Create the private config directory used by Claude for one project.

    No host-wide Claude configuration is imported.  A user explicitly signs
    in through Claude's own flow on the machine that owns this memory.
    """
...
def claude_environment(project_id: str, base: dict[str, str]) -> dict[str, str]:
    """Return an environment pinned to this project's Claude configuration."""
    environment = dict(base)
    environment["CLAUDE_CONFIG_DIR"] = str(ensure_project_claude_config_dir(project_id))
    return environment
