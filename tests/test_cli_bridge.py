...
def accept_explicit_test_project_claims(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cli_bridge, "resolve_codex_executable", lambda **_kwargs: cli_bridge.shutil.which("codex"),
    )
    original_which = cli_bridge.shutil.which
    monkeypatch.setattr(
        cli_bridge.shutil, "which",
        lambda name: str(tmp_path / name) if name in {"codex", "claude"} else original_which(name),
    )

    def installation(provider):
        executable = cli_bridge.shutil.which(provider)
        if not executable:
            raise cli_bridge.CliExecutionError("dependency_unavailable", "test client unavailable")
        return ClientInstallation(
            provider, "unknown", tmp_path / f"{provider}-config", ClientCommand(Path(executable)),
        )

    monkeypatch.setattr(cli_bridge.CliRunner, "_installation", staticmethod(installation))
    create_codex_home = project_secrets.ensure_project_codex_home
    monkeypatch.setattr(
        project_secrets, "ensure_project_codex_home",
        lambda project_id, **_kwargs: create_codex_home(project_id, import_global_auth=False),
    )
...
@pytest.mark.parametrize("identity", [None, {}, {"id": "other", "name": "Wrong"}, {"id": "p1", "name": "Orchard"}])
def test_setup_prefers_exact_server_name_and_supports_older_servers(monkeypatch, tmp_path, identity):
    project = {"id": "p1", "name": "Bootstrap", "api_port": 18001, "web_port": 20001}
    monkeypatch.setattr(cli_bridge.SetupService, "_project_config", staticmethod(lambda _: (tmp_path, project)))
    monkeypatch.setattr(cli_bridge, "binding_from_project", lambda *_: SimpleNamespace(api_url="http://127.0.0.1:18001", project_id="p1"))
    monkeypatch.setattr(cli_bridge.ProjectHttpClient, "headers", lambda _: {})
    def get(url, **kwargs):
        if url.endswith("/health"):
            return SimpleNamespace(status_code=200)
        if identity is None:
            raise httpx.ConnectError("offline")
        return SimpleNamespace(status_code=200 if identity else 404, json=lambda: identity)
    monkeypatch.setattr(cli_bridge.httpx, "get", get)
    result = cli_bridge.SetupService._project_status(tmp_path)
    assert result["ready"] is True
    assert result["name"] == ("Orchard" if identity and identity.get("id") == "p1" else "Bootstrap")
...
def test_codex_runner_uses_ephemeral_schema_output(monkeypatch):
    monkeypatch.setattr(cli_bridge, "resolve_codex_executable", lambda **_kwargs: "/bin/codex")
...
def test_codex_runner_pins_project_scoped_codex_home(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda provider, **kwargs: SimpleNamespace(ready=provider == "codex"),
    )
    monkeypatch.setattr(cli_bridge.subprocess, "run", run)
    result = cli_bridge.CliRunner().generate(
        {
...
def test_sleep_runner_prefers_the_project_subscription_and_reports_it(monkeypatch):
    calls = []
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
        lambda provider, **kwargs: SimpleNamespace(ready=provider == "claude"),
    )
    monkeypatch.setattr(
        cli_bridge,
        "claude_environment",
        lambda project_id, base: {**base, "CLAUDE_CONFIG_DIR": f"/private/{project_id}/claude"},
    )
    monkeypatch.setattr(
        cli_bridge,
        "codex_environment",
        lambda project_id, base: {**base, "CODEX_HOME": f"/private/{project_id}/codex"},
    )
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_claude",
        staticmethod(
            lambda prompt, schema, timeout, project_id=None, **_kwargs: (
                calls.append(("claude", project_id)) or '{"topics": []}',
                "",
                {"usage": {}, "model": "claude-test"},
            )
        ),
    )
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_codex",
        staticmethod(lambda *args: (_ for _ in ()).throw(AssertionError("Codex must not run"))),
    )
    ...
    assert calls == [("claude", "project-one")]
    assert result["provider"] == "claude"
    assert result["executor_preference"] == "claude"
    assert "fallback_from" not in result


def test_sleep_runner_falls_back_before_call_when_preferred_subscription_is_unavailable(monkeypatch):
    calls = []
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
    ...
    monkeypatch.setattr(cli_bridge, "claude_environment", lambda _project_id, base: base)
    monkeypatch.setattr(cli_bridge, "codex_environment", lambda _project_id, base: base)
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_codex",
        staticmethod(
            lambda prompt, schema, timeout, project_id=None, **_kwargs: (
                calls.append(("codex", project_id)) or '{"topics": []}',
                "",
                {"usage": {}, "model": "codex-test"},
            )
        ),
    )
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_claude",
        staticmethod(lambda *args: (_ for _ in ()).throw(AssertionError("Claude must not run"))),
    )
    ...
    assert calls == [("codex", "project-one")]
    assert result["provider"] == "codex"
    assert result["executor_preference"] == "claude"
    assert result["fallback_from"] == "claude"


def test_sleep_runner_does_not_retry_invalid_model_output_with_another_subscription(monkeypatch):
    calls = []
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
        lambda _provider, **kwargs: SimpleNamespace(ready=True),
    )
    monkeypatch.setattr(cli_bridge, "claude_environment", lambda _project_id, base: base)
    monkeypatch.setattr(cli_bridge, "codex_environment", lambda _project_id, base: base)
    ...
    def invalid(*_args, **_kwargs):
        calls.append("claude")
        raise cli_bridge.CliExecutionError("invalid_model_output", "bad structure")

    monkeypatch.setattr(cli_bridge.CliRunner, "_run_claude", staticmethod(invalid))
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_codex",
        staticmethod(lambda *args: calls.append("codex")),
    )

    with pytest.raises(cli_bridge.CliExecutionError, match="bad structure"):
...
def test_sleep_runner_does_not_spend_another_subscription_after_a_rate_limit(monkeypatch):
    calls = []
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
        lambda _provider, **_kwargs: SimpleNamespace(ready=True),
    )
    monkeypatch.setattr(cli_bridge, "claude_environment", lambda _project_id, base: base)
    monkeypatch.setattr(cli_bridge, "codex_environment", lambda _project_id, base: base)
    ...
    def rate_limited(*_args, **_kwargs):
        calls.append("claude")
        raise cli_bridge.CliExecutionError("rate_limited", "try later")

    monkeypatch.setattr(cli_bridge.CliRunner, "_run_claude", staticmethod(rate_limited))
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_run_codex",
        staticmethod(lambda *_args: calls.append("codex")),
    )

    with pytest.raises(cli_bridge.CliExecutionError, match="try later"):
        cli_bridge.CliRunner().generate(
            {
                "provider": "claude",
                "project_id": "project-one",
...
def test_setup_saves_a_private_embeddings_key_and_starts_one_official_login(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
        lambda provider, **_kwargs: SimpleNamespace(ready=False, as_dict=lambda: {"ready": False}),
    )
    monkeypatch.setattr(cli_bridge.shutil, "which", lambda name: f"/bin/{name}")
    calls = []
    process = SimpleNamespace(poll=lambda **_kwargs: None)
    monkeypatch.setattr(
        cli_bridge.subprocess,
        "Popen",
...
def test_setup_reports_project_readiness_and_resumes_only_connected_provider(monkeypatch, tmp_path):
    ...
    project_secrets.ensure_project_claude_config_dir("p1")
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
    ...
    monkeypatch.setattr(
        cli_bridge,
        "codex_hook_status",
        lambda root, **_kwargs: SimpleNamespace(as_dict=lambda: {"ready": False}),
    )
    monkeypatch.setattr(
        cli_bridge,
    ...
    monkeypatch.setattr(
        ...
        lambda url, **kwargs: (
            ...
            or SimpleNamespace(raise_for_status=lambda **_kwargs: None, json=lambda: {"scheduled": 1})
        ),
    )
    monkeypatch.setattr(
        cli_bridge.httpx,
        "get",
        lambda *args, **kwargs: SimpleNamespace(
            status_code=200, raise_for_status=lambda **_kwargs: None,
    ...
    # The sleep host is Claude here.  A Codex chat becomes ready as soon as
    # its own lifecycle adapter is authorized; it must not require a second
    # Codex subscription merely because the active conversation is Codex.
    monkeypatch.setattr(
        cli_bridge,
        "codex_hook_status",
        lambda root, **_kwargs: SimpleNamespace(as_dict=lambda: {"ready": True}),
    )
    assert cli_bridge.SetupService().status(str(root))["ready_for_client"] == {
        "claude": True,
        "codex": True,
    }
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            status_code=200,
            raise_for_status=lambda **_kwargs: None,
            json=lambda: {"state": "connection_required", "executor_provider": "claude"},
        ),
    )
    assert setup.resume_memory(str(root), "claude") == {"resumed": True, "provider": "claude"}
    # A page refresh must not submit the same auth-paused job a second time.
    assert setup.resume_memory(str(root), "claude") == {"resumed": False, "provider": "claude"}
...
def test_setup_auto_resume_selects_one_ready_subscription_in_preferred_order(monkeypatch):
    setup = cli_bridge.SetupService()
    monkeypatch.setattr(setup, "_project_id", lambda _root: "p1")
    monkeypatch.setattr(
        setup,
        "_memory_status",
        lambda _root: {"state": "connection_required", "executor_provider": "claude"},
    )
    monkeypatch.setattr(
        setup,
        ...
        lambda provider, **_kwargs: SimpleNamespace(
            ready=True,
            as_dict=lambda: {"ready": True, "provider": provider, "reason": "authenticated"},
        ),
    )
    monkeypatch.setattr(setup, "_codex_hook_status", lambda _root: None)
    monkeypatch.setattr(
        setup,
        "_docker_status",
        lambda: {"installed": True, "running": True, "ready": True},
    )
    monkeypatch.setattr(setup, "_embeddings_ready", lambda _project_id: True)
    monkeypatch.setattr(setup, "_project_status", lambda _root: {"ready": True})
    monkeypatch.setattr(setup, "_backup_status", lambda _root: {"ready": True})
    monkeypatch.setattr(
        cli_bridge,
        "claude_statusline_status",
        lambda _root: {"ready": True, "installed": True, "reason": "ready"},
    )

    status = setup.status("/project")

    assert status["clients"]["claude"]["resume_ready"] is True
    assert status["clients"]["codex"]["resume_ready"] is False
...
def test_setup_status_exposes_only_safe_native_auth_state(monkeypatch, tmp_path):
    ...
    setup._auth["claude"] = cli_bridge.AuthAttempt(
        provider="claude", process=SimpleNamespace(poll=lambda **_kwargs: None)
    )
...
def test_runner_reports_missing_cli_and_invalid_contract(monkeypatch):
    ...
    monkeypatch.setattr(cli_bridge, "resolve_codex_executable", lambda **_kwargs: None)
...
def test_backup_supplement_is_project_scoped_bounded_and_authenticated(monkeypatch, tmp_path):
    ...
    from dduo_solo_founder.client_binding import binding_from_project
    from dduo_solo_founder.project_config import load_project

    binding = binding_from_project(root, load_project(root), require_approval=False)
        ...
        json.dumps(
            {
                ...
                "client": "codex",
                "session_external_id": "backup-fixture",
                "queued": [],
            }
        )
    )
    ...
    assert result["claude_auth"] == {"included": False, "credential_store": "unavailable"}
    ...
    assert "sleep_auth_unavailable" in incomplete_credentials["warnings"]
    assert "openai_key_unavailable" in incomplete_credentials["warnings"]

    claude = project_secrets.project_claude_config_dir(project_id)
    claude.mkdir(parents=True)
    (claude / ".credentials.json").write_text('{"oauthAccount":{"emailAddress":"test@example.com"}}')
    claude_only = cli_bridge.build_backup_supplement(project_id)
    assert claude_only["claude_auth"] == {"included": True, "credential_store": "file"}
    assert "sleep_auth_unavailable" not in claude_only["warnings"]
    assert "secrets/claude/.credentials.json" in {
        item["path"] for item in claude_only["files"]
    }
...
def test_setup_service_checks_safe_edge_states_and_starts_mac_docker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        cli_bridge,
        "subscription_auth_status",
        lambda provider, **_kwargs: SimpleNamespace(ready=True, as_dict=lambda: {"ready": True}),
    )
...
def test_start_auth_invalidates_every_qualified_readiness_cache_entry(monkeypatch):
    setup = cli_bridge.SetupService()
    setup._auth_status = {
        "codex:host:/managed/codex::/managed/config:cli": (0.0, object()),
        "codex:host:/other/codex::/other/config:vscode": (0.0, object()),
        "claude:host:/managed/claude::/managed/config:cli": (0.0, object()),
    }
    monkeypatch.setattr(
        setup,
        "_subscription_status",
    ...
    monkeypatch.setattr(setup, "_memory_status", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(
        cli_bridge.CliRunner,
        "_installation",
        staticmethod(
            lambda _provider: ClientInstallation(
                "codex", "unknown", Path("/synthetic/config"), ClientCommand(Path("/bin/true"))
            )
        ),
    )
    monkeypatch.setattr(cli_bridge, "resolve_codex_executable", lambda **_kwargs: "/bin/true")
    monkeypatch.setattr(
        cli_bridge.subprocess,
        "Popen",
    ...
    assert setup.start_auth("codex") == {"status": "waiting", "provider": "codex"}
    assert all(not key.startswith("codex:host:") for key in setup._auth_status)
    assert "claude:host:/managed/claude::/managed/config:cli" in setup._auth_status


@pytest.mark.parametrize("provider", ["codex", "claude"])
def test_sleep_attempt_keeps_the_authenticated_runtime_and_private_environment(monkeypatch, tmp_path, provider):
    selected = ClientInstallation(
        provider, "vscode", tmp_path / "chat-config", ClientCommand(tmp_path / "standalone"),
        ClientCommand(tmp_path / "extension-probe"),
    )
    alternate = "claude" if provider == "codex" else "codex"
    alternate_installation = ClientInstallation(
        alternate, "cli", tmp_path / "other-config", ClientCommand(tmp_path / "other"),
    )
    resolutions = []

    def installation(candidate):
        resolutions.append(candidate)
        assert resolutions.count(candidate) == 1, "runtime was rediscovered after its authentication check"
        return selected if candidate == provider else alternate_installation

    monkeypatch.setattr(cli_bridge.CliRunner, "_installation", staticmethod(installation))
    monkeypatch.setattr(
        cli_bridge, "resolve_codex_executable",
        lambda **kwargs: str(kwargs["installation"].management_command.executable),
    )
    key = "CODEX_HOME" if provider == "codex" else "CLAUDE_CONFIG_DIR"
    private_environment = {key: str(tmp_path / "private-sleep"), "PATH": ""}
    monkeypatch.setattr(
        cli_bridge.CliRunner, "_provider_environment",
        staticmethod(lambda _provider, _project_id: dict(private_environment)),
    )
    checked = {}

    def auth(candidate, **kwargs):
        if candidate == provider:
            checked.update(kwargs)
        return SimpleNamespace(ready=candidate == provider)

    monkeypatch.setattr(cli_bridge, "subscription_auth_status", auth)
    executed = {}

    def run(command, **kwargs):
        executed.update(command=command, environment=kwargs["env"])
        if provider == "codex":
            Path(command[command.index("--output-last-message") + 1]).write_text('{"topics":[]}')
            output = ""
        else:
            output = '{"structured_output":{"topics":[]}}'
        return SimpleNamespace(returncode=0, stdout=output, stderr="")

    monkeypatch.setattr(cli_bridge.subprocess, "run", run)
    assert cli_bridge.CliRunner().generate({
        "provider": provider, "project_id": "synthetic-project", "instructions": "Segment",
        "schema": SCHEMA, "input": {},
    })["ok"]
    assert checked["installation"] is selected
    assert executed["command"][0] == str(selected.management_command.executable)
    assert executed["environment"] == checked["environment"] == private_environment


def test_setup_hook_cache_tracks_exact_managed_descriptor_and_never_uses_legacy_fallback(monkeypatch, tmp_path):
    current = ClientInstallation("codex", "cli", tmp_path / "a", ClientCommand(tmp_path / "codex-a"))
    selected = [current]
    probes = []
    monkeypatch.setattr(cli_bridge.CliRunner, "_installation", staticmethod(lambda _provider: selected[0]))
    monkeypatch.setattr(
        cli_bridge, "codex_hook_status",
        lambda _root, *, installation: probes.append(installation) or cli_bridge.CodexHookStatus(True, "authorized", 3, ["trusted"]),
    )
    setup = cli_bridge.SetupService()
    assert setup._codex_hook_status(tmp_path).ready
    assert setup._codex_hook_status(tmp_path).ready
    selected[0] = ClientInstallation("codex", "cli", tmp_path / "b", current.management_command)
    assert setup._codex_hook_status(tmp_path).ready
    assert probes == [current, selected[0]]
    monkeypatch.setattr(
        cli_bridge.CliRunner, "_installation",
        staticmethod(lambda _provider: (_ for _ in ()).throw(cli_bridge.CliExecutionError("dependency_unavailable", "missing"))),
    )
    assert setup._codex_hook_status(tmp_path).reason == "check_failed"
    assert len(probes) == 2
