...
def test_client_readiness_command_reports_ready_and_protected_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        launcher,
        "inspect_client_readiness",
        lambda client, root: (_ for _ in ()).throw(ValueError("client must be codex or claude")),
    )
    assert (
        runner.invoke(
            launcher.app,
...
def test_remote_host_orchestrates_isolated_stack_and_gateway(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(launcher, "_require_remote_codex_auth", lambda _: None)
...
def test_remote_codex_auth_is_a_preflight_not_a_runtime_surprise(monkeypatch, tmp_path):
    home = tmp_path / "codex"
    monkeypatch.setattr(launcher, "ensure_project_codex_home", lambda _: home)
    monkeypatch.setattr(
        launcher,
        "codex_environment",
    ...
    def status(client, *, environment):
        seen.update({"client": client, "environment": environment})
        return SimpleNamespace(ready=False, reason="login_required")

    monkeypatch.setattr(launcher, "subscription_auth_status", status)
    with pytest.raises(RuntimeError, match="Codex subscription authentication") as error:
        launcher._require_remote_codex_auth("p1")
    assert seen["client"] == "codex"
    assert seen["environment"]["CODEX_HOME"] == str(home)
    ...
    assert launcher._require_remote_codex_auth("p1") is None
...
def test_full_recovery_supplement_restores_only_project_scoped_state(monkeypatch, tmp_path):
    ...
    (hooks / "p1-codex-session.json").write_text('{"project_id":"p1"}')
    ...
    assert result == {"codex_auth": True, "hook_spools": 1, "mcp_observations": 1}
    assert (codex_home / "auth.json").stat().st_mode & 0o777 == 0o600
    assert (hook_target / "p1-codex-session.json").is_file()
...
def test_full_recovery_supplement_validates_everything_before_writing(monkeypatch, tmp_path):
    extracted = tmp_path / "extracted"
    (extracted / "secrets/codex").mkdir(parents=True)
    (extracted / "secrets/dduo.env").write_text("OPENAI_API_KEY=key\n")
    (extracted / "secrets/codex/auth.json").write_text('{malformed')
    hooks = extracted / "host-state/hooks"
    hooks.mkdir(parents=True)
    (hooks / "p1-codex-session.json").write_text('{"project_id":"p1"}')
    ...
    with pytest.raises(BackupError, match="Codex auth.json is malformed"):
...
def test_full_recovery_restores_project_state_but_ignores_machine_updater_state(
    ...
    (hooks / "p1-codex-session.json").write_text(
        '{"project_id":"p1","binding_id":"old-binding"}'
    )
    (extracted / "host-state/mcp-observability.json").write_text(
    ...
    assert result == {"codex_auth": True, "hook_spools": 1, "mcp_observations": 2}
    assert saved == [("p1", {"OPENAI_API_KEY": "key"})]
    assert remote_credential.read_text() == "remote-project-token\n"
    restored_hooks = list(hook_target.glob("p1-codex-session-restored-*.json"))
