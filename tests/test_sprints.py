...
def test_sprint_migration_preserves_preexisting_history_and_enforces_ownership():
    ...
    # Later migrations may extend the schema. The sprint migration remains a
    # required ancestor rather than permanently being the repository head.
    revisions = {
        revision.revision
        for revision in ScriptDirectory(str(source.parents[1])).walk_revisions()
    }
