...
def test_bounded_reader_discards_partial_oversized_and_invalid_utf8_records(tmp_path, monkeypatch):
    monkeypatch.setattr(codex_telemetry, "MAX_JSONL_RECORD_BYTES", 8)
    monkeypatch.setattr(codex_telemetry, "TRANSCRIPT_READ_CHUNK_BYTES", 3)
    path = tmp_path / "synthetic.jsonl"
    content = b"partial\n" + b"x" * 25 + b"\n\xff\n{}\n\xff"
    path.write_bytes(content)
    state = codex_telemetry._JsonlScanState()
    descriptor = os.open(path, os.O_RDONLY | getattr(os, "O_BINARY", 0))
    try:
        assert list(codex_telemetry._bounded_jsonl_lines(
            descriptor, start=2, end=len(content), scan_state=state,
        )) == ["{}"]
        assert state.ambiguity_epoch == 4
        for start, end in [(-1, 2), (3, 2)]:
            with pytest.raises(ValueError, match="invalid transcript range"):
                list(codex_telemetry._bounded_jsonl_lines(descriptor, start=start, end=end))
    finally:
        os.close(descriptor)


@pytest.mark.parametrize("target", ["gpt-6-astra", {"id": "gpt-6-astra"}, {"name": "gpt-6-astra"}])
def test_cursor_tracks_reroutes_but_not_events_after_completed_turn(target):
    events = [
        {"type": "turn_context", "payload": {"turn_id": "t", "model": "gpt-5.6-terra"}},
        {"type": "model_reroute", "payload": {"target": target}},
        {"type": "event_msg", "payload": {"type": "task_complete", "turn_id": "t"}},
        {"type": "model_reroute", "payload": {"target": "must-not-be-attributed"}},
    ]
    counters, model, saw_token, trustworthy = codex_telemetry._cursor_tail_state(
        ["invalid json", *(json.dumps(event) for event in events)],
        turn_id="t", scan_state=codex_telemetry._JsonlScanState(),
    )
    assert model == "gpt-6-astra"
    assert counters is None and not saw_token and not trustworthy


def test_transcript_open_rejects_file_replaced_during_open(tmp_path, monkeypatch):
    path = tmp_path / "synthetic.jsonl"
    path.write_text("{}\n", encoding="utf-8")
    ...
    def replaced(candidate):
        ...
        if calls == 1:
            return status
        values = list(status)
        values[1] += 1  # A replacement inode is not the file we opened.
        return os.stat_result(values)

    monkeypatch.setattr(codex_telemetry.os, "lstat", replaced)
    assert codex_telemetry._open_regular_transcript(path) is None


@pytest.mark.parametrize("error,status", [
    (OSError("temporarily unreadable"), "retryable_unavailable"),
    (ValueError("invalid record"), "terminal_unavailable"),
    (UnicodeError("invalid encoding"), "terminal_unavailable"),
])
def test_transcript_io_errors_never_report_zero_usage_or_leak_descriptor(tmp_path, monkeypatch, error, status):
    path = tmp_path / "synthetic.jsonl"
    path.write_text("{}\n", encoding="utf-8")
    descriptor = 12345  # Stubbed open: never a real process file descriptor.
    closed = []
    monkeypatch.setattr(codex_telemetry, "_open_regular_transcript", lambda _: (descriptor, path.stat()))
    ...
    monkeypatch.setattr(codex_telemetry, "_bounded_jsonl_lines", broken_read)
    monkeypatch.setattr(codex_telemetry.os, "close", close_once)
    result = codex_telemetry.read_codex_transcript_usage_result(path, session_id="s", turn_id="t")
    assert result.status == status
    assert codex_telemetry.capture_codex_transcript_cursor(path, session_id="s", turn_id="t") is None
