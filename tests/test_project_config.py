...
def test_registry_lock_acquires_empty_file_then_reuses_its_inode(
    tmp_path: Path,
    monkeypatch,
):
    registry = tmp_path / "registry.json"
...
def test_registry_lock_never_writes_before_acquiring_even_with_stale_empty_stat(tmp_path: Path, monkeypatch):
    registry = tmp_path / "registry.json"
    real_try_lock = project_config._try_lock_descriptor
    real_truncate = os.ftruncate
    real_write = project_config._write_descriptor
    held = False
    attempts = 0
    ...
    def acquire(fd):
        ...
        attempts += 1
        if attempts == 1:
            # Model a contender holding an otherwise empty lock file.
            return False
        held = real_try_lock(fd)
        return held

    def truncate(fd, size):
        assert held, "a contender must not truncate the lock holder's file"
        return real_truncate(fd, size)

    def write(fd, value):
        assert held, "lock metadata can be written only by its owner"
        return real_write(fd, value)

    monkeypatch.setattr(project_config, "_try_lock_descriptor", acquire)
    monkeypatch.setattr(project_config.os, "ftruncate", truncate)
    monkeypatch.setattr(project_config, "_write_descriptor", write)
    with project_config.portable_file_lock(registry):
        assert held and attempts == 2
