...
def test_claude_nonobject_auth_status_is_a_safe_diagnostic_failure(monkeypatch, payload):
    monkeypatch.setattr(client_readiness.shutil, "which", lambda name: f"/bin/{name}")
    monkeypatch.setattr(
        client_readiness.subprocess, "run",
        lambda *_args, **_kwargs: SimpleNamespace(returncode=0, stdout=json.dumps(payload), stderr=""),
    )
    status = client_readiness.subscription_auth_status("claude")
    assert not status.ready and status.reason == "check_failed"
...
@pytest.mark.parametrize("surface,path", [("cli", ["/hooks"]), ("vscode", None)])
def test_hook_review_instructions_match_surface_without_certifying_chat(
    monkeypatch, tmp_path, surface, path
):
    installation = ClientInstallation("codex", surface, tmp_path, ClientCommand(tmp_path / "codex"))
    monkeypatch.setattr(
        client_readiness, "subscription_auth_status",
        lambda *_args, **_kwargs: client_readiness.SubscriptionAuthStatus("codex", True, "authenticated", "codex login"),
    )
    monkeypatch.setattr(
        client_readiness, "codex_hook_status",
        lambda *_args, **_kwargs: client_readiness.CodexHookStatus(False, "authorization_required", 3, ["untrusted"]),
    )
    result = client_readiness.client_readiness("codex", tmp_path, installation=installation)
    action = result["actions"][0]
    assert action["path"] == path
    assert "Settings" not in action["detail"]
    assert result["verification_scope"] == "installation"
    assert result["lifecycle_verified"] is None
    if surface == "vscode":
        assert action["review_action"] is None and action["trust_action"] is None
        assert "has not been verified" in action["detail"]
...
def test_auth_uses_selected_config_but_explicit_sleep_environment_takes_precedence(monkeypatch, tmp_path, family, config_key):
    installation = ClientInstallation(
        family, "cli", tmp_path / "selected", ClientCommand(tmp_path / family),
    )
    monkeypatch.setattr(client_readiness, "_codex_version", lambda *_args, **_kwargs: (0, 150, 0))
    monkeypatch.setattr(client_readiness, "detached_cli_environment", lambda: {config_key: "/ambient/config"})
    captured = []
    ...
    def run(command, **kwargs):
        ...
        return SimpleNamespace(
            returncode=0, stderr="",
            stdout="Logged in using ChatGPT" if family == "codex" else
            '{"loggedIn":true,"authMethod":"claude.ai","apiProvider":"firstParty"}',
        )

    monkeypatch.setattr(client_readiness.subprocess, "run", run)
    assert client_readiness.subscription_auth_status(family, installation=installation).ready
    assert captured[-1][1][config_key] == str(installation.config_dir)
    sleep_env = {config_key: str(tmp_path / "private-sleep")}
    assert client_readiness.subscription_auth_status(family, installation=installation, environment=sleep_env).ready
    assert captured[-1][1] == sleep_env


def test_explicit_probe_checks_its_own_version_and_hooks_without_changing_login_runtime(monkeypatch, tmp_path):
    installation = ClientInstallation(
        "codex", "vscode", tmp_path / "config",
        ClientCommand(tmp_path / "standalone"), ClientCommand(tmp_path / "extension"),
    )
    versions = []
    monkeypatch.setattr(
        client_readiness, "_codex_version",
        lambda executable, **_kwargs: versions.append(executable) or (0, 150, 0),
    )
    process = FakeProcess('\n'.join([
        json.dumps({"id": 1, "result": {}}), json.dumps({"id": 2, "result": {"data": []}}),
    ]))
    captured = []
    ...
    monkeypatch.setattr(client_readiness.subprocess, "Popen", popen)
    client_readiness._codex_app_server_request("hooks/list", project_root=tmp_path, installation=installation)
    assert versions == [str(installation.probe_command.executable)]
    assert captured[0][0] == [str(installation.probe_command.executable), "app-server", "--stdio"]
    assert captured[0][1]["CODEX_HOME"] == str(installation.config_dir)
    assert client_readiness.resolve_codex_executable(installation=installation) == str(installation.management_command.executable)
    assert versions[-1] == str(installation.management_command.executable)
