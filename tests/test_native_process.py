...
def test_windows_claude_npm_uses_declared_node_entrypoint_without_shell(monkeypatch, tmp_path):
    monkeypatch.setattr(native_process, "_windows", lambda: True)
    monkeypatch.setattr(native_process.shutil, "which", lambda _: "C:/Program Files/nodejs/node.exe")
    ...
    assert native_process.client_command(str(tmp_path / "claude.cmd"), arguments, client="claude") == [
        "C:/Program Files/nodejs/node.exe", str(script), *arguments,
    ]
