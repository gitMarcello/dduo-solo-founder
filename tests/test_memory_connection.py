...
def isolate_native_checks(monkeypatch):
...
def test_native_status_notices_distinguish_consent_from_unverified_capture(
    ...
    monkeypatch.setattr(
        memory_connection, "codex_hook_status", lambda _root: _status(reason, ready=ready)
    )
    ...
    if ready:
        ...
        assert "Settings > Hooks" in checked["next_action"]
...
def test_warning_identity_survives_same_issue_but_changes_after_recovery(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(memory_connection, "codex_hook_status", lambda _root: native.status)
...
def test_warning_scope_separates_roots_and_bindings_and_cache_is_bounded(monkeypatch, tmp_path):
    probes = []
    monkeypatch.setattr(
        memory_connection, "codex_hook_status", lambda root: probes.append(root) or _status()
    )
...
async def test_sdk_pending_notice_prevents_mutation_and_acknowledgement_is_per_call(
    ...
    monkeypatch.setattr(memory_connection, "codex_hook_status", lambda _root: _status())
