...
def test_native_installer_bootstrap_update_rollback_hooks_and_mcp(tmp_path):
    # Never inherit a developer's selected vendor profile into the disposable
    # smoke. The synthetic CLI asserts that registration uses its private home.
    external_claude = tmp_path / "external Claude profile"
    external_claude.mkdir()
    external_settings = external_claude / "settings.json"
    external_settings.write_text('{"sentinel": "untouched"}', encoding="utf-8")
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "PYTHONUTF8": "1", "PYTHONIOENCODING": "utf-8",
             "CLAUDE_CONFIG_DIR": str(external_claude)},
        capture_output=True, text=True, encoding="utf-8", timeout=180,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert external_settings.read_text(encoding="utf-8") == '{"sentinel": "untouched"}'
