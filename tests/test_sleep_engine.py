...
def test_generated_language_validation_rejects_only_clear_opposites():
    with pytest.raises(SleepProviderError, match="wrong language"):
        sleep_engine._validate_generated_language(
            "The release must always preserve database safety.",
            "it",
            field="memory text",
        )
    sleep_engine._validate_generated_language(
        "Android release API v2",
        "it",
        field="a topic label",
    )
...
async def seed_pending(db, *, client="codex", off_record=False):
    ...
    turn = Turn(
        ...
        user_prompt="I must test Android as a real user.",
...
async def test_sleep_rejects_clearly_wrong_memory_text_language(db_factory):
    async with db_factory() as db:
        project, session, turn = await seed_pending(db)
        turn.user_prompt = "La release deve sempre preservare la sicurezza del database."
        job = await schedule_sleep(
            db, project_id=project.id, session_id=session.id, trigger="manual"
        )
        await db.commit()

        class EnglishTextWithItalianTagProvider(SuccessfulProvider):
            async def generate(self, **kwargs):
                result = await super().generate(**kwargs)
                topic = result["topics"][0]
                topic["language"] = "it"
                topic["label"] = "Android"
                topic["query_text"] = "Android"
                return result

        assert (
            await process_one_sleep_job(db, NoHits(), EnglishTextWithItalianTagProvider(turn.id))
            is True
        )
        await db.refresh(job)
...
async def test_sleep_rejects_model_language_enum_that_conflicts_with_user(db_factory):
    async with db_factory() as db:
        project, session, turn = await seed_pending(db)
        turn.user_prompt = "La release deve sempre preservare la sicurezza del database."
        job = await schedule_sleep(
...
async def test_setup_normalizes_legacy_jobs_to_the_server_executor(db_factory):
    async with db_factory() as db:
        ...
        assert {job.provider for job in resumed} == {"codex"}
...
async def test_session_start_resumes_waiting_jobs_and_queues_unassigned_turns(db_factory):
    async with db_factory() as db:
        ...
        assert {item.provider for item in resumed} == {"codex"}
...
async def test_sleep_helper_empty_and_project_filter_paths(db_factory):
    async with db_factory() as db:
        ...
        await db.commit()
        assert job.provider == "codex" and job.source_client == "claude"
