...
async def test_member_cannot_attach_events_or_context_to_another_members_turn(
    ...
    try:
        capture = await client.get(
            f"/projects/{project_id}/sessions/{manager_session.id}/capture-status",
            headers=headers,
            params={"client": "codex", "turn_id": turn_id},
        )
...
async def test_manual_versions_capabilities_briefing_and_actor(db_factory, monkeypatch):
    ...
    member_headers = {"Authorization": f"Bearer {member_token}"}
    try:
        rename = {"name": "Orchard", "expected_version": 1}
        assert (await client.post(f"/projects/{project_id}/rename", headers=member_headers, json=rename)).status_code == 403
        assert (await client.post(f"/projects/{uuid.uuid4()}/rename", headers=manager_headers, json=rename)).status_code == 404
        assert (await client.get(f"/projects/{uuid.uuid4()}/identity", headers=manager_headers)).status_code == 404
        assert (await client.post(f"/projects/{other_project_id}/rename", headers=manager_headers, json=rename)).status_code in {403, 404}
        assert (await client.get(f"/projects/{other_project_id}/identity", headers=manager_headers)).status_code in {403, 404}
        assert (await client.post(f"/projects/{project_id}/rename", headers=manager_headers, json=rename)).status_code == 200
        assert (await client.get(f"/projects/{project_id}/identity", headers=member_headers)).json()["name"] == "Orchard"
        update = {
