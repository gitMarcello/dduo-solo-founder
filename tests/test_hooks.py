...
def local_state_path(tmp_path: Path, client: str, external_session_id: str) -> Path:
    """Build the same binding-qualified private ledger path as the hook."""
    binding = hooks.binding_from_project(
        tmp_path,
        project(),
        require_approval=False,
        enforce_project_claim=False,
    )
    return hooks.state_path("p1", client, external_session_id, binding.binding_id)
...
def test_distinct_external_session_ids_never_share_a_spool_filename(monkeypatch, tmp_path):
    ...
    first = hooks.state_path("p1", "codex", "topic/a", "b" * 64)
    second = hooks.state_path("p1", "codex", "topic?a", "b" * 64)
    assert first != second


def test_unconfigured_session_asks_once_for_activation(monkeypatch, tmp_path):
    output = hook_input(monkeypatch, {"cwd": str(tmp_path), "session_id": "repair-session"})
    hooks.session_start()
...
def test_declined_session_and_unconfigured_prompts_are_silent(monkeypatch, tmp_path):
    ...
    assert messages[0] == {"continue": True}
    assert "cannot identify this chat session" in messages[1]["hookSpecificOutput"]["additionalContext"]
...
def test_unconfigured_prompt_is_not_claimed_as_queued(monkeypatch, tmp_path):
    ...
    hooks.user_prompt_submit()
    context = json.loads(output.getvalue())["hookSpecificOutput"]["additionalContext"]
    assert "cannot identify this chat session" in context
    assert "queued" not in context
...
def test_session_start_surfaces_expired_auth_before_opening_setup(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda transport, project_id, **_kwargs: telemetry_flushes.append(project_id),
    )
    calls = []
    ...
    state = hooks.read_state(local_state_path(tmp_path, "claude", "s1"))
...
def test_steering_prompts_keep_one_turn_and_distinct_delivery_observations(monkeypatch, tmp_path):
    ...
    state = hooks.read_state(local_state_path(tmp_path, "codex", "thread"))
...
def test_rejected_late_prompt_does_not_leave_context_for_the_next_turn(monkeypatch, tmp_path):
    ...
    hooks.user_prompt_submit()
    state = hooks.read_state(local_state_path(tmp_path, "codex", "session"))
...
def test_repeated_identical_session_start_contexts_are_distinct_deliveries(monkeypatch, tmp_path):
    ...
    state = hooks.read_state(local_state_path(tmp_path, "codex", "same"))
    observations = state["pending_context_observations"]
    assert len(observations) == 2
...
def test_stop_uses_private_turn_id_and_queues_failed_commit(monkeypatch, tmp_path):
    ...
    assert calls == [
        (
            "http://127.0.0.1:8765/turns/private-turn/stop-check",
            {
...
def test_late_stop_never_closes_a_newer_active_turn(monkeypatch, tmp_path, offline):
    monkeypatch.setattr(hooks, "HOOK_STATE_DIR", tmp_path / "state")
    monkeypatch.setattr(hooks, "load_project", lambda _: project())
    path = local_state_path(tmp_path, "codex", "session")
    active = {
...
def test_flush_spooled_turns_replays_idempotently(monkeypatch, tmp_path):
    ...
    calls = []
    responses = iter(
        [Response({"id": "session"}), Response({"turn": {"id": "turn"}}), Response({"committed": True})]
    )
...
def test_hook_helpers_handle_invalid_input_artifacts_and_retry_queues(monkeypatch, tmp_path):
    ...
    assert state["pending_commits"][0]["turn_id"] == "a"
    assert state["pending_commits"][0]["payload"] == {"assistant_response": "new"}
    assert state["pending_commits"][0]["revision"]
...
def test_hook_recovery_paths_cover_tty_setup_failures_and_provider_notice_cleanup(
    ...
    monkeypatch.setattr(hooks.httpx, "post", lambda *args, **kwargs: Response({"committed": True}))
    ...
    output = hook_input(
        monkeypatch,
        {"cwd": str(tmp_path), "session_id": "repair-session"},
    )
    hooks.session_start()
    ...
    output = hook_input(
        monkeypatch,
        {"cwd": str(tmp_path), "client": "claude", "session_id": "connection-session", "prompt": "Resume"},
    )
    hooks.user_prompt_submit()
    rendered = json.loads(output.getvalue())["hookSpecificOutput"]["additionalContext"]
...
def test_unchanged_briefing_reuses_manual_without_reinjecting_it(monkeypatch, tmp_path):
    ...
    path = hooks.state_path("p1", "codex", "s1", binding.binding_id)
    hooks.write_state(path, state)
...
def test_remote_outage_uses_only_the_last_authenticated_manual(monkeypatch, tmp_path, event):
    ...
    assert process_calls == []
    state = hooks.read_state(hooks.state_path("remote", "codex", "remote-session", binding.binding_id))
    observation = state["pending_context_observations"][-1]
...
def test_remote_outage_does_not_repeat_manual_already_present_in_live_session(
    ...
    path = hooks.state_path("remote", "codex", "remote-session", binding.binding_id)
    hooks.write_state(path, state)
...
def test_hook_state_merge_honors_an_explicit_short_lock_budget(monkeypatch, tmp_path):
    observed: list[float] = []

    @contextmanager
    def lock(_path, *, timeout):
        observed.append(timeout)
        yield

    monkeypatch.setattr(hooks, "portable_file_lock", lock)
    hooks.merge_state_update(tmp_path / "state.json", {}, {"answer": "saved"}, lock_timeout=1)
