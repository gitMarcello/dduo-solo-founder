...
async def test_capture_status_is_exact_and_never_returns_turn_content(api_client):
    client, _ = api_client
    ...
    checked = await client.get(
        f"/projects/{project['id']}/sessions/{session['id']}/capture-status",
        params={"client": "codex", "turn_id": turn["id"]},
    )
    assert checked.status_code == 200
    body = checked.json()
    assert set(body) == {"session", "turn"}
    assert set(body["session"]) == {"id", "external_id", "client", "off_record", "started_at"}
    ...
    rendered = str(body)
    assert "Remember the Android release state" not in rendered
    assert "Captured" not in rendered
    ...
    unrelated_project = project_payload()
    assert (await client.post("/projects", json=unrelated_project)).status_code == 200
    wrong_project = await client.get(
        f"/projects/{unrelated_project['id']}/sessions/{session['id']}/capture-status",
        params={"client": "codex"},
    )
    ...
    unrelated_session = (
        await client.post(
            ...
            json={"client": "codex", "external_id": "other-session"},
        )
    ).json()
    wrong_turn = await client.get(
        f"/projects/{project['id']}/sessions/{unrelated_session['id']}/capture-status",
        params={"client": "codex", "turn_id": turn["id"]},
    )
    assert wrong_turn.status_code == 404
    exact_empty = await client.get(
        f"/projects/{project['id']}/sessions/{unrelated_session['id']}/capture-status",
        params={"client": "codex"},
    )
    assert exact_empty.json()["turn"] is None
...
async def test_memory_status_hides_cli_output_and_opens_setup_without_exposing_token(
    ...
    assert "Connect Codex or Claude" in status["summary"]
