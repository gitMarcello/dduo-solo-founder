...
def test_headless_preparation_is_browserless_and_resumable(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(launcher, "_require_remote_sleep_auth", authentications.append)
...
def test_codex_device_login_uses_project_private_auth(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(launcher, "resolve_client_installation", lambda *_args, **_kwargs: SimpleNamespace(
        config_dir=tmp_path / "synthetic-client", management_command=SimpleNamespace(node_executable=None),
    ))
    calls = []
