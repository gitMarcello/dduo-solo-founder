...
def test_agent_contract_is_concise_open_and_task_preferring():
    ...
    assert "authoritative project host owns one Codex sleep executor" in normalized_skill
    assert "eight pending turns" in normalized_skill
    assert "app restart is required only after installing or updating Codex" in normalized_skill
    assert "fresh chat or session is required after installation" in normalized_skill
...
def test_installer_warns_but_succeeds_when_post_commit_legacy_cleanup_fails(
    ...
    if not shutil.which("node"):
        return
    env, log, _ = _write_codex_installer_stubs(tmp_path, path_version="0.150.0")
    legacy_checks = tmp_path / "legacy-checks"
    codex = tmp_path / "bin/codex"
    codex.write_text(
        f'''#!/bin/sh
echo "path-codex $*" >> "{log}"
...
if [ "$1" = "plugin" ] && [ "$2" = "list" ] && [ "$3" = "--json" ]; then
  count=0
  if [ -f "{legacy_checks}" ]; then read count < "{legacy_checks}"; fi
  count=$((count + 1))
  printf '%s\\n' "$count" > "{legacy_checks}"
  if [ "$count" -ge 3 ]; then
    echo '{{"installed":[{{"pluginId":"opendduo@personal"}}],"available":[]}}'
...
'''
    ...
    assert legacy_checks.read_text().strip() == "3"
    runtime = tmp_path / ".local/share/dduo-solo-founder/runtime"
    assert runtime.exists()
...
'''
    ...
    assert commands.count("codex plugin add") == 3
