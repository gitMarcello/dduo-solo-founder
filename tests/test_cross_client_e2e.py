...
from dduo_solo_founder.models import Memory, Project, SleepJob
...
async def test_claude_sleep_codex_recall_and_shared_task_round_trip(api_client, db_factory):
    ...
    async with db_factory() as db:
        ...
        assert job is not None and job.provider == "claude" and job.status == "completed"
        assert job.executor_preference == "claude"
        assert job.source_client == "claude"
        stored_project = await db.get(Project, project["id"])
        assert stored_project is not None and stored_project.sleep_executor_preference == "claude"
