...
def test_codex_home_imports_only_file_auth_and_pins_file_storage(
    ...
    project_id = "11111111-1111-4111-8111-111111111111"
    home = project_secrets.ensure_project_codex_home(
        project_id, import_global_auth=True, source_codex_home=fake_home / ".codex"
    )
...
def test_codex_auth_import_uses_only_explicit_profile_and_never_replaces_private_login(
    monkeypatch, isolated_secrets: Path, tmp_path: Path
):
    fake_home = tmp_path / "home"
    default_profile = fake_home / ".codex"
    selected_profile = tmp_path / "selected-client"
    for directory, token in ((default_profile, "default-B"), (selected_profile, "selected-A")):
    ...
    project_id = "11111111-1111-4111-8111-111111111111"
    home = project_secrets.ensure_project_codex_home(
        project_id, import_global_auth=True, source_codex_home=selected_profile
    )
    assert (home / "auth.json").read_text() == "selected-A"
    project_secrets.ensure_project_codex_home(
        project_id, import_global_auth=True, source_codex_home=default_profile
    )
    assert (home / "auth.json").read_text() == "selected-A"


def test_periodic_codex_environment_never_imports_ambient_or_selected_profile(
    monkeypatch, isolated_secrets: Path, tmp_path: Path
):
    monkeypatch.setattr(
        project_secrets, "_read_external_private_text",
        lambda *_args, **_kwargs: pytest.fail("runtime checks must not import client credentials"),
    )
    project_id = "11111111-1111-4111-8111-111111111111"
    home = project_secrets.ensure_project_codex_home(project_id)
    environment = project_secrets.codex_environment(project_id, {"CODEX_HOME": str(tmp_path / "client")})
    assert environment["CODEX_HOME"] == str(home)
    assert not (home / "auth.json").exists()


@pytest.mark.parametrize("source", [None, Path("relative")])
def test_codex_auth_import_requires_explicit_absolute_profile(isolated_secrets: Path, source):
    with pytest.raises(ValueError, match="explicit absolute source"):
        project_secrets.ensure_project_codex_home(
            "11111111-1111-4111-8111-111111111111",
            import_global_auth=True, source_codex_home=source,
        )
