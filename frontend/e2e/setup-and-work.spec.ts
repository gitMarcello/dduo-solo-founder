...
test('local Setup keeps secrets local, delegates Codex hook trust, and installs or repairs Claude telemetry', async ({
  ...
  await expect(page.getByText('Completa solo ciò che richiede il tuo intervento')).toBeVisible();
  ...
  await expect(page.getByText('Complete only the items that need you')).toBeVisible();
  await page.reload();
