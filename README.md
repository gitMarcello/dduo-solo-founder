...
embeddings (semantic search) and **one** supported subscription login: Codex
or Claude. The first supported project chat establishes the preferred sleep
provider; before each consolidation the memory host verifies that preference
and can use the other already-verified host subscription only if the preferred
one is unavailable. Collaborators supply their own chat client access and use
the host's memory services.
...
- [VS Code lifecycle acceptance record](docs/vscode-lifecycle-validation.md)
...
real graphical-chat acceptance check before they can be called supported: hook
readiness and a manifest alone do not prove lifecycle delivery. Other agent
clients remain disabled. See [verification boundaries](docs/platform-support.md).
